$(function callChange() {
    ...
    DRP_FIELD = $('.ShowChild4').find("select[id*='DRP_FIELD']").toArray();
    for (a = 0; a < DRP_FIELD.length; ++a) {
        showChild(DRP_FIELD[a]);
    }
...
function closestCallChange(element) {
    ...
    DRP_FIELD = $(element).find("select[id*='DRP_FIELD']").toArray();
    for (a = 0; a < DRP_FIELD.length; ++a) {
        showChild(DRP_FIELD[a]);
    }

    TXT_FIELD = $(element).find("input[id*='TXT_FIELD']").toArray();
    for (a = 0; a < TXT_FIELD.length; ++a) {
        showChild(TXT_FIELD[a]);
    }

    TXT_FIELD = $(element).find("textarea[id*='TXT_FIELD']").toArray();
    for (a = 0; a < TXT_FIELD.length; ++a) {
        showChild(TXT_FIELD[a]);
    }

    CHK_FIELD1 = $(element).find("input[id*='CHK_FIELD']:not(:checked)").closest('span').toArray();
    for (c = 0; c < CHK_FIELD1.length; ++c) {
        showChild(CHK_FIELD1[c]);
    }

    CHK_FIELD = $(element).find("input[id*='CHK_FIELD']:checked").closest('span').toArray();
    for (c = 0; c < CHK_FIELD.length; ++c) {
        showChild(CHK_FIELD[c]);
    }

    RAD_FIELD1 = $(element).find("input[id*='RAD_FIELD']:not(:checked)").toArray();
    for (d = 0; d < RAD_FIELD1.length; ++d) {
        showChild(RAD_FIELD1[d]);
    }

    RAD_FIELD = $(element).find("input[id*='RAD_FIELD']:checked").closest('span').toArray();
    for (d = 0; d < RAD_FIELD.length; ++d) {
        showChild(RAD_FIELD[d]);
    }

    CONTROLTYPE = $(element).find("span[id*='lblCONTROLTYPE']").closest('span').toArray();
    for (e = 0; e < CONTROLTYPE.length; ++e) {
        SetHeader(CONTROLTYPE[e]);
    }
...
function HideHeaderIcons(element) {
    ...
    $(element).closest('tr').find('td').eq(12).addClass('disp-none');
