function checkOnChangeCrit_Editable(element, variablename, tblPVID, tblRECID, tblTABLENAME, tblFIELDNAME) {
    ...
    $("#MainContent_grdOnPageSpecs tr").each(function () {
                ...
                if (grd_VARIABLENAME == variablename || Formulas.indexOf(variablename) != -1) {
                    ...
                    for (i = 1; i < formualArr.length; i++) {
                        ...
                        if (formualArr[i].indexOf(']') != -1) {
                            ...
                            if ($(element).closest('tr').find('td').find('.' + DerVariablename).hasClass('radio')) {

                                DerVALUE = $(element).closest('tr').find('td').find('.' + DerVariablename).closest('td').find('.' + DerVariablename + ' :checked').next().text();

                            }
                            else {
                                DerVALUE = $(element).closest('tr').find('td').find('.' + DerVariablename + '').val();
                            }

                            $.ajax({
                                type: "POST",
                                url: "AjaxFunction_DM.aspx/CheckDatatype",
                                data: '{Val: "' + DerVALUE + '"}',
                        ...
                        for (i = 1; i < CritCodeArr.length; i++) {
                            ...
                            if (CritCodeArr[i].indexOf(']') != -1) {
                                ...
                                if ($('.' + CritCodeVariablename).hasClass('radio')) {
                                    ...
                                    CritCodeVALUE = $(element).closest('tr').find('td').find('.' + CritCodeVariablename).closest('td').find('.' + CritCodeVariablename + ' :checked').next().text();

                                }
                                else {
                                    CritCodeVALUE = $(element).closest('tr').find('td').find('.' + CritCodeVariablename + '').val();
                                }
                    ...
                    if (ISDERIVED == 'True') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "AjaxFunction_DM.aspx/EXECUTE_FORMULA",
                            ...
                            success: function (data) {
                                if (data.d == 'Object reference not set to an instance of an object.') {
                                    ...
                                    if ($(element).closest('tr').find('td').find('.' + SETFIELDID).hasClass('radio')) {

                                        $(element).closest('tr').find('td').find('.' + SETFIELDID).closest('td').find('span:textEquals(' + data.d + ')').find('input').prop('checked', true);

                                    }
                                    else {
                                        $(element).closest('tr').find('td').find('.' + SETFIELDID + '').val(data.d);
                                    }

                                    DATA_Changed_Editable_Set_Value_Data(SETFIELDID, tblPVID, tblRECID, tblTABLENAME, SET_VALUE_DATA);
                                }
                            },
                            failure: function (response) {
                                if (response.d == 'Object reference not set to an instance of an object.') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "AjaxFunction_DM.aspx/CHECK_CONDITION",
                                ...
                                if (data.d == 'Object reference not set to an instance of an object.') {
                                    ...
                                    if (data.d == '0') {
                                        ...
                                        if (ERR_MSG != '' && grd_VARIABLENAME == variablename) {
                                            ...
                                            var encodedMsg = btoa(ERR_MSG);
                                            // Decode and display the message
                                            var decodedMsg = atob(encodedMsg);
                                        ...
                                        if (SET_VALUE_DATA != '') {
                                            ...
                                            if ($(element).closest('tr').find('td').find('.' + SETFIELDID).hasClass('radio')) {
                                                ...
                                                $(element).closest('tr').find('td').find('.' + SETFIELDID).closest('td').find('span:textEquals(' + SET_VALUE_DATA + ')').find('input').prop('checked', true);

                                            }
                                            else {
                                                $(element).closest('tr').find('td').find('.' + SETFIELDID + '').val(SET_VALUE_DATA);
                                            }
    ...
    if (msg == true) {
        ...
        DATA_Changed_Editable(element, variablename, tblPVID, tblRECID, tblTABLENAME, tblFIELDNAME);
