...
namespace CTMS
{
    public partial class NIWRS_KITS_SITE_TRANSF : System.Web.UI.Page
    {
        DAL dal = new DAL();
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count == 1)
                    {
                        ...
                        ddlToSite.DataSource = ds.Tables[0];
                        ddlToSite.DataValueField = "INVID";
                        ddlToSite.DataBind();
                    }
                    else
                    {
                        ...
                        ddlToSite.DataValueField = "INVID";
                        ddlToSite.DataBind();
                        ddlToSite.Items.Insert(0, new ListItem("--Select--", "0"));
                    }
                }
            }
            catch (Exception ex)
        ...
        private void GET_KITS_SITE_TRANSF()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_KITS_SITE_TRANSF", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        private void GET_NEW_ORDERID_SITE_TRANSF()
        {
            try
            {
                if (ddlCountry.SelectedValue != "0" && ddlSite.SelectedValue != "0")
                {
                    DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_NEW_ORDERID_SITE_TRANSF", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlToSite.SelectedValue, ID: ddlSite.SelectedValue);
                ...
                for (int i = 0; i < gvKITS.Rows.Count; i++)
                {
                    ...
                    if (Chk_Sel.Checked)
                    {
                        ...
                        dal_IWRS.IWRS_KITS_SITE_SP
                           (
                           ACTION: "GENERATE_ORDER_SITE_TRANSF",
                           ID: ID.Value,
                           COUNTRYID: ddlCountry.SelectedValue,
                           SITEID: ddlToSite.SelectedValue,
                           KITNO: ddlSite.SelectedValue,
        ...
        private void SEND_MAIL()
        {
            try
            {
                ...
                DataSet ds = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAILS", STRATA: "KIT_COUNTRY", COUNTRY: ddlCountry.SelectedItem.Text);
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE_TRANSF");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[TOSITE]", ddlToSite.SelectedItem.Text);
                    ...
                    BODY = BODY.Replace("[TOSITE]", ddlToSite.SelectedItem.Text);
