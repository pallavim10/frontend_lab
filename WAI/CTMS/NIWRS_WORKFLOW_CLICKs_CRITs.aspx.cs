...
namespace CTMS
{
    public partial class NIWRS_WORKFLOW_CLICKs_CRITs : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    DataSet ds = dal_IWRS.NIWRS_SETUP_SP(ACTION: "SELECT_LIST_ADDFIELD", ID: Request.QueryString["ID"].ToString());

                    lblList.Text = ds.Tables[0].Rows[0]["LISTNAME"].ToString() + " | " + ds.Tables[0].Rows[0]["FIELDNAME"].ToString();
                    GET_ADD_CRIT_FIELDS(ds.Tables[0].Rows[0]["LISTID"].ToString(), "Listing");
                    GET_REVIEW_STATUS();
                    GET_STATUS();
                    GET_VISIT();
                    GET_SETFIELDS();
        ...
        private void GET_ADD_CRIT_FIELDS(string ID, string TYPE)
        {
            try
            {
                DataSet ds = dal_IWRS.NIWRS_SETUP_SP(ACTION: "GET_ADD_CRIT_FIELDS", ID: ID, FIELDNAME: TYPE, LISTID: ID);
        ...
        private void INSERT_FIELD_CRIT()
        {
            try
            {
                string
        ...
        private void INSERT_SETFIELD(string ID)
        {
            try
            {
                if (ddlStatus.SelectedIndex != 0)
                {
                    dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "INSERT_SETFIELD_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString(), STEP_CRIT_ID: ID, FIELDNAME: "STATUS", VALUE: ddlStatus.SelectedValue);
                }
                else
                {
                    dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "DELETE_SETFIELD_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString(), STEP_CRIT_ID: ID, FIELDNAME: "STATUS");
                }
                ...
                for (int i = 0; i < repeatSetFields.Items.Count; i++)
                {
                    ...
                    if (txtSetFieldVal.Text != "")
                    {
                        dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "INSERT_SETFIELD_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString(), STEP_CRIT_ID: ID, FIELDNAME: hfCOLNAME.Value, VALUE: txtSetFieldVal.Text);
                    }
                    else
                    {
                        dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "DELETE_SETFIELD_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString(), STEP_CRIT_ID: ID, FIELDNAME: hfCOLNAME.Value);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void DELETE_SETFIELD_ALL(string ID)
        {
            try
            {
                dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "DELETE_SETFIELD_FIELD_ALL", STEPID: ID);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        private void UPDATE_CRIT()
        {
            ...
            CritCodeQUERY = null, CritCodeQUERY1 = null, CritCodeQUERY2 = null, CritCodeQUERY3 = null, CritCodeQUERY4 = null, CritCodeQUERY5 = null;
                ...
                DataSet ds = dal_IWRS.NIWRS_WORKFLOW_SP(
                                ACTION: "UPDATE_FIELD_CRIT",
                                ID: ViewState["editStepFieldCritID"].ToString(),
                ...
                INSERT_SETFIELD(ViewState["editStepFieldCritID"].ToString());

                INSERT_EMAIL(Request.QueryString["ID"].ToString(), ViewState["editStepFieldCritID"].ToString());
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        private void SELECT_CRIT(string ID)
        {
            try
            {
                ...
                DataSet ds = dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "SELECT_FIELD_CRIT", ID: ID);
                ...
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "bindOptionValues();ShowEmailDiv();", true);

                if (ds.Tables[0].Rows[0]["SETFIELD"].ToString() != "")
                {
                    DataSet dsSet = dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "GET_STEP_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString(), STEP_CRIT_ID: ID);
                ...
                GET_FIELD_CRIT_EMAIL(Request.QueryString["ID"].ToString(), ID);
        ...
        private void DELETE_CRIT(string ID)
        {
            try
            {
                dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "DELETE_FIELD_CRIT", ENTEREDBY: Session["USER_ID"].ToString()
,ID: ID);
            }
            catch (Exception ex)
            {
        ...
        private void GET_CRIT()
        {
            try
            {
                DataSet ds = dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "GET_FIELD_CRIT", STEPID: Request.QueryString["ID"].ToString());
        ...
        private void GET_FIELD_CRIT_EMAIL(string FIELID, string FIELID_CRIT_ID)
        {
            try
            {
                DataSet ds = dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "GET_FIELD_CRIT_EMAIL", STEPID: FIELID, STEP_CRIT_ID: FIELID_CRIT_ID);
        ...
        private void INSERT_EMAIL(string FIELDID, string FIELD_CRIT_ID)
        {
            try
            {
                    ...
                    for (int i = 0; i < gvEmailds.Rows.Count; i++)
                    {
                        ...
                        if (txtEMAILIDs.Text != "")
                        {
                            dal_IWRS.NIWRS_WORKFLOW_SP
                                (
                                ACTION: "INSERT_FIELD_CRIT_EMAIL",
                                STEPID: FIELDID,
                                STEP_CRIT_ID: FIELD_CRIT_ID,
                ...
                else
                {
                    dal_IWRS.NIWRS_WORKFLOW_SP
                        (
                        ACTION: "DELETE_ALL_FIELD_CRIT_EMAIL",
                        STEPID: FIELDID,
                        STEP_CRIT_ID: FIELD_CRIT_ID
                        );
                }
        ...
        protected void grdStepCrits_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                ViewState["editStepFieldCritID"] = id;
