<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="DM_Approve_UnFreeze.aspx.cs" Inherits="CTMS.DM_Approve_UnFreeze" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function countCheckboxes() {
            ...
            if (count < 1) {
                alert("Select at least one data to Approve UnFreeze Request");
                event.preventDefault();
            }
        }

        function countCheckboxesDisapprove() {
            var inputElems = document.getElementsByTagName("input");
            var count = 0;
            for (var i = 0; i < inputElems.length; i++) {
                if (inputElems[i].type === "checkbox" && inputElems[i].checked === true) {
            ...
            if (count < 1) {
                alert("Select at least one data to Disapprove UnFreeze Request");
                event.preventDefault();
            }
        }

        function Check_child_ReqApprove(element) {

            $(element).closest('tr').next().find('td').find('table').find('input[type=checkbox][id*=Chk_ReqApp_Child]').each(function () {
        ...
        function Check_child_ReqDisApprove(element) {

            $(element).closest('tr').next().find('td').find('table').find('input[type=checkbox][id*=Chk_ReqDisApp_Child]').each(function () {
                if ($(element).prop('checked') == true) {
                    $(this).prop('checked', true);
                }
                else {
        ...
        function VIEW_DM_DATA(element) {
            ...
            if ((($(element).closest('table').attr('id')).indexOf('Gridchild') != -1)) {
                ...
                MODULENAME = $(element).closest('table').closest('tr').prev().find('td:eq(16)').find('span').text();
            }
            else {
                ...
                MODULENAME = $(element).closest('tr').find('td:eq(16)').find('span').text();
            }

            var test = "DM_DATA.aspx?MODULEID=" + MODULEID + "&VISITID=" + VISITID + "&PVID=" + PVID + "&RECID=" + RECID + "&TABLENAME=" + TABLENAME + "&MODULENAME=" + MODULENAME;
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="box box-warning">
        <div class="box-header">
            <h3 class="box-title">Request List for Unfreeze
            </h3>
        </div>
        <div class="lblError">
        ...
        <div class="box-body">
            <div class="form-group ">
                <div style="width: 100%; overflow: auto; padding-top: 10px;">
                    <asp:GridView ID="Grd_Freeze" runat="server" AutoGenerateColumns="False" CellPadding="3"
                        CssClass="table table-bordered table-striped" CellSpacing="2" OnRowDataBound="Grd_Freeze_RowDataBound" HeaderStyle-ForeColor="Blue">
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="Approve" ItemStyle-CssClass="txt_center">
                                <HeaderTemplate>
                                    <asp:Button ID="Btn_Approve" runat="server" OnClick="Btn_Update_Click" Style="border: none; text-align: center"
                                        CssClass="btn btn-primary btn-sm cls-btnSave1" Text="Approve" OnClientClick="return countCheckboxes();" />
                                </HeaderTemplate>
                                <ItemTemplate>
                                    <asp:CheckBox ID="Chk_ReqApp" runat="server" onclick="Check_child_ReqApprove(this)" />
                                </ItemTemplate>
                            </asp:TemplateField>
                            <asp:TemplateField HeaderText="Disapprove" ItemStyle-CssClass="txt_center">
                                <HeaderTemplate>
                                    <asp:Button ID="Btn_Disapprove" runat="server" Style="border: none; text-align: center"
                                        CssClass="btn btn-primary btn-sm cls-btnSave1" OnClick="Btn_DisApprove_Click"
                                        Text="Disapprove" OnClientClick="return countCheckboxesDisapprove();" />
                                </HeaderTemplate>
                                <ItemTemplate>
                                    <asp:CheckBox ID="Chk_ReqDisApp" runat="server" onclick="Check_child_ReqDisApprove(this)" />
                                </ItemTemplate>
                            </asp:TemplateField>
                            ...
                            <asp:TemplateField HeaderStyle-CssClass="align-left">
                                <HeaderTemplate>
                                    <label>UnFrozen Request Details</label><br />
                                    <label style="color: blue; font-weight: lighter; margin-bottom: 0px;">[UnFrozen Request By]</label><br />
                                    <label style="color: green; font-weight: lighter; margin-bottom: 0px;">[Datetime(Server)]</label><br />
                                    <label style="color: red; font-weight: lighter; margin-bottom: 0px;">[Datetime(User), (Timezone)]</label>
                                </HeaderTemplate>
                                <ItemTemplate>
                                    <div>
                                        <div>
                                            <asp:Label ID="UNFREEZE_REQBYNAME" runat="server" Text='<%# Bind("UNFREEZE_REQBYNAME") %>' ForeColor="Blue"></asp:Label>
                                        </div>
                                        <div>
                                            <asp:Label ID="UNFREEZE_REQ_CAL_DAT" runat="server" Text='<%# Bind("UNFREEZE_REQ_CAL_DAT") %>' ForeColor="Green"></asp:Label>
                                        </div>
                                        <div>
                                            <asp:Label ID="UNFREEZE_REQ_CAL_TZDAT" runat="server" Text='<%# Bind("UNFREEZE_REQ_CAL_TZDAT") %>' ForeColor="Red"></asp:Label>
                                        </div>
                                    </div>
                                </ItemTemplate>
                            </asp:TemplateField>
                                                    ...
                                                    <div style="width: 100%; overflow: scroll;">
                                                        <asp:GridView ID="Gridchild" runat="server" CellPadding="3" AutoGenerateColumns="false" HeaderStyle-ForeColor="Maroon"
                                                            ...
                                                            <Columns>
                                                                ...
                                                                <asp:TemplateField HeaderText="Approve" ItemStyle-CssClass="txt_center" HeaderStyle-Width="50px">
                                                                    <ItemTemplate>
                                                                        <asp:CheckBox ID="Chk_ReqApp_Child" runat="server" />
                                                                        <asp:Label ID="lblQuery" runat="server" ForeColor="Red" Visible="false"> </asp:Label>
                                                                    </ItemTemplate>
                                                                </asp:TemplateField>
                                                                <asp:TemplateField HeaderText="Disapprove" ItemStyle-CssClass="txt_center" HeaderStyle-Width="50px">
                                                                    <ItemTemplate>
                                                                        <asp:CheckBox ID="Chk_ReqDisApp_Child" runat="server" />
                                                                    </ItemTemplate>
                                                                </asp:TemplateField>
                                                                <asp:TemplateField HeaderText="View" ItemStyle-CssClass="txt_center">
                                                                    <ItemTemplate>
