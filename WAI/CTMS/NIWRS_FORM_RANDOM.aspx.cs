...
namespace CTMS
{
    public partial class NIWRS_FORM_RANDOM : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    DataSet ds = dal_IWRS.NIWRS_WORKFLOW_SP(ACTION: "CHECK_SCREENING_EXISTS");
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        hdnCurrentDate.Value = Session["IWRS_CurrentDate"].ToString();
                    }
                    else
                    {
                        Response.Redirect("NIWRS_FORM_SCRRAND.aspx?STEPID=" + Request.QueryString["STEPID"].ToString() + "&HEADER=" + Request.QueryString["HEADER"].ToString() + "");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void GET_AVL_RAND_NUM()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_AVL_SP(ACTION: "GET_AVL_RAND_NUM", SUBJID: Request.QueryString["SUBJID"].ToString());
        ...
        private void CHECK_AVL_RAND_NUM()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_AVL_SP(ACTION: "CHECK_AVL_RAND_NUM", SUBJID: Request.QueryString["SUBJID"].ToString());

                if (ds.Tables[0].Rows.Count < 1)
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Ranodmization number not available'); window.location.href = 'IWRS_Status_Dashboard.aspx'; ", true);
                }
