...
namespace CTMS
{
    public partial class eTMF_Snapshot_ETMF : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void gvArtifact_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_Snapshot_SP(ACTION: "get_BD_Docs_FileStructure_INTERNAL",
                    RefNo: MainRefNo,
                    DocTypeId: DocTypeId,
                    SnapId: Request.QueryString["SNAPID"].ToString()
                    );
        ...
        private void GET_FILES_LIST()
        {
            try
            {
                DataSet ds = dal.eTMF_Snapshot_SP(ACTION: "GET_FILES_SNAP",
                ZoneID: ddlZone.SelectedValue,
                SectionId: ddlSections.SelectedValue,
                SnapId: Request.QueryString["SNAPID"].ToString()
                );
