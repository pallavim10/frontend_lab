...
namespace CTMS
{
    public partial class NIWRS_KITS_COUNTRY_PENDING : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_COUNTRY_ORDERS()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_COUNTRY_SHIPMENT", COUNTRYID: ddlCountry.SelectedValue);
        ...
        protected void gvOrders_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_COUNTRY_SHIPMENT_KITS", ORDERID: ORDERID);
        ...
        protected void gvOrders_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName != "")
                {
                    ...
                    if (SHIPMENTID.Text.Trim() == "" || SHIPMENTID.Text.Trim() == null)
                    {
                        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Please Enter Shipment ID.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                    }
                    else if (SHIPMENTDATE.Text.Trim() == "" || SHIPMENTDATE.Text.Trim() == null)
                    {
                        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Please Enter Shipment Date.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                    }
                    else if (SHIPMENTID.Text.Trim() != "" && SHIPMENTDATE.Text.Trim() != "")
                    {
                        if (e.CommandName == "Accept")
                        {
                            dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "UPDATE_COUNTRY_SHIPMENT", ORDERID: e.CommandArgument.ToString(), SHIPMENTID: SHIPMENTID.Text, SHIPMENTDATE: SHIPMENTDATE.Text, TABLENAME: TABLENAME.Text);

                            SEND_MAIL(e.CommandArgument.ToString(), COUNTRYNAME.Text, SHIPMENTID.Text, SHIPMENTDATE.Text);
        ...
        protected void gvKits_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                Label TABLENAME = (Label)gvOrders.Rows[parentindex].FindControl("TABLENAME");
                Label ORDERID = (Label)gvOrders.Rows[parentindex].FindControl("ORDERID");

                if (e.CommandName == "Quarantine")
                {
                    dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "QUARANTINE_COUNTRY_KIT", KITNO: e.CommandArgument.ToString(), TABLENAME: TABLENAME.Text);

                    SEND_MAIL_QUARANTINE(e.CommandArgument.ToString(), COUNTRYNAME.Text, ORDERID.Text);
                ...
                {
                    dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "BLOCK_COUNTRY_KIT", KITNO: e.CommandArgument.ToString(), TABLENAME: TABLENAME.Text);

                    SEND_MAIL_BLOCK(e.CommandArgument.ToString(), COUNTRYNAME.Text, ORDERID.Text);
                ...
                {
                    dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "DAMAGE_COUNTRY_KIT", KITNO: e.CommandArgument.ToString(), TABLENAME: TABLENAME.Text);

                    SEND_MAIL_DAMAGE(e.CommandArgument.ToString(), COUNTRYNAME.Text, ORDERID.Text);
                ...
                {
                    dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "RETURN_COUNTRY_KIT", KITNO: e.CommandArgument.ToString(), TABLENAME: TABLENAME.Text);

                    SEND_MAIL_RETURN(e.CommandArgument.ToString(), COUNTRYNAME.Text, ORDERID.Text);
                ...
                {
                    dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "REJECT_COUNTRY_KIT", KITNO: e.CommandArgument.ToString(), TABLENAME: TABLENAME.Text);

                    SEND_MAIL_REJECT(e.CommandArgument.ToString(), COUNTRYNAME.Text, ORDERID.Text);

                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Kit marked as Reject.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                }
        ...
        private void SEND_MAIL(string ORDERID, string COUNTRY, string SHIPMENTID, string SHIPMENTDATE)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_COUNTRY_PENDING");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[SHIPDAT]", SHIPMENTDATE);
                    ...
                    BODY = BODY.Replace("[SHIPDAT]", SHIPMENTDATE);
        ...
        private void SEND_MAIL_QUARANTINE(string KITNO, string COUNTRY, string ORDERID)
        {
            try
            {
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[QUARANTINED_AT]", "Country Order");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_BLOCK(string KITNO, string COUNTRY, string ORDERID)
        {
            try
            {
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[Blocked_AT]", "Country Order");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_DAMAGE(string KITNO, string COUNTRY, string ORDERID)
        {
            try
            {
                ...
                string EMAILIDS = "", CCEMAILIDS = "", BCCEMAILIDS = "", SUBJECT = "", BODY = "";
                DataSet ds = dal_IWRS.NIWRS_SETUP_SP(ACTION: "GET_EMAILS", STRATA: "KIT_CENTRAL_DAMAGE");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[Damaged_AT]", "Country Order");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_RETURN(string KITNO, string COUNTRY, string ORDERID)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_COUNTRY_PENDING - Return");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[Returned_AT]", "Country Order");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_REJECT(string KITNO, string COUNTRY, string ORDERID)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_COUNTRY_PENDING - Reject");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[Rejected_AT]", "Country Order");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void ExportSingleSheet()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_COUNTRY_SHIPMENT_EXPORT", COUNTRYID: ddlCountry.SelectedValue);
