...
namespace CTMS
{
    public partial class NIWRS_LIST_PREDOSING : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_LIST_DETAILS(string SOURCE_ID)
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_LIST_SP(ACTION: "GET_LIST_DETAILS_PREDOSING", LISTID: SOURCE_ID, SITEID: ddlSite.SelectedValue, SUBSITEID: ddlSubSite.SelectedValue, ENTEREDBY: Session["User_ID"].ToString());
        ...
        protected void gridData_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                for (int i = 0; i < e.Row.Cells.Count; i++)
                {
                    ...
                    if (gridDataFields[i].ToString() == "Last Visit")
                    {
                        if (IsDosing(e.Row.Cells[2].Text.ToString(), e.Row.Cells[i].Text.ToString()))
                        {
                            e.Row.Cells[i].Attributes.Add("onclick", "ShowKits(this);");
                            e.Row.Cells[i].CssClass = e.Row.Cells[i].CssClass + " fontBlue";
                        }
                    }
                }
        ...
        private bool IsDosing(string INDICATION, string VISIT)
        {
            try
            {
                bool result = false;

                DataSet ds = dal_IWRS.IWRS_SET_VISIT_SP(ACTION: "CHECK_VISIT_DOSING", INDICATION: INDICATION, VISIT: VISIT);
