...
namespace CTMS
{
    public partial class DM_OpenCRF_ReadOnly : System.Web.UI.Page
    {
        ...
        protected void Grd_OpenCRF_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "GOTOPAGE")
                {
                    ...
                    {
                        Response.Redirect("DM_DataEntry_MultipleData_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID);
                    }
                    else
                    {
                        Response.Redirect("DM_DataEntry_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID);
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void lbtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Write("<script> alert('Rules successfully refreshed.');window.location='DM_OpenCRF_ReadOnly.aspx?VISITNUM=" + drpVisit.SelectedValue + "&SUBJID=" + drpSubID.SelectedValue + "'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
