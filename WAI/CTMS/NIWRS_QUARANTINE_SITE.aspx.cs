...
namespace CTMS
{
    public partial class NIWRS_QUARANTINE_SITE : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_KITS()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_QUARANTINE_SITE", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        protected void gvKits_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                Label TREAT_GRP_NAME = (Label)gvKits.Rows[index].FindControl("TREAT_GRP_NAME");
                Label Location = (Label)gvKits.Rows[index].FindControl("Location");

                if (e.CommandName == "UnQuarantine")
                {
                    dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "UNQUARANTINE_SITE_KIT", KITNO: e.CommandArgument.ToString(), ORDERID: ORDERID.Text);

                    SEND_MAIL_UNQUARANTINE(TREAT_GRP_NAME.Text, COUNTRYNAME.Text, SITEID.Text, Location.Text);
                ...
                {
                    dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "REJECT_SITE_KIT", KITNO: e.CommandArgument.ToString());

                    SEND_MAIL_REJECT(TREAT_GRP_NAME.Text, COUNTRYNAME.Text, SITEID.Text, Location.Text);
                ...
                else if (e.CommandName == "Destroy")
                {
                    dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "DESTROY_SITE_KIT", KITNO: e.CommandArgument.ToString());

                    SEND_MAIL_DESTROY(TREAT_GRP_NAME.Text, COUNTRYNAME.Text, SITEID.Text, Location.Text);
        ...
        private void SEND_MAIL_UNQUARANTINE(string KITNO, string COUNTRY, string SITE, string Location)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_SITE - UnQuarantine");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[RELEASED_AT]", Location);
        ...
        private void SEND_MAIL_REJECT(string KITNO, string COUNTRY, string SITE, string Location)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                string EMAILIDS = "", CCEMAILIDS = "", BCCEMAILIDS = "", SUBJECT = "", BODY = "";
                DataSet ds = dal_IWRS.NIWRS_SETUP_SP(ACTION: "GET_EMAILS", STRATA: "KIT_SITE_REJECT", SITEID: SITE);
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_SITE - Reject");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[SITE]", SITE);
                    SUBJECT = SUBJECT.Replace("[KITNO]", KITNO);
                    SUBJECT = SUBJECT.Replace("[REJECTED_AT]", Location);
                    SUBJECT = SUBJECT.Replace("[USERNAME]", Session["User_Name"].ToString());
                    SUBJECT = SUBJECT.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    SUBJECT = SUBJECT.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
        ...
        private void SEND_MAIL_DESTROY(string KITNO, string COUNTRY, string SITE, string Location)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_SITE - Destroy");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[SITE]", SITE);
                    SUBJECT = SUBJECT.Replace("[KITNO]", KITNO);
                    SUBJECT = SUBJECT.Replace("[DESTROYED_AT]", Location);
                    SUBJECT = SUBJECT.Replace("[USERNAME]", Session["User_Name"].ToString());
                    SUBJECT = SUBJECT.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    SUBJECT = SUBJECT.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
        ...
        private void SEND_MAIL_RETURN(string KITNO, string COUNTRY, string SITE, string Location)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_SITE - Return");
        ...
        protected void btnSubmitReject_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    {
                        dal_IWRS.IWRS_KITS_SITE_SP(
                            ACTION: "REJECT_SITE_KIT",
                            KITNO: KITNO,
                            REJECTCOMM: txtRejectComments.Text,
                            TABLENAME: TABLENAME
                            );

                        SEND_MAIL_REJECT(KITNO, COUNTRYNAME, SITEID, Location);
                    }
                }
        ...
        private void ExportSingleSheet()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_QUARANTINE_SITE_EXPORT", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
