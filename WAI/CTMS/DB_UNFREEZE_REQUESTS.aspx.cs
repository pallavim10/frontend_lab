...
namespace CTMS
{
    public partial class DB_UNFREEZE_REQUESTS : System.Web.UI.Page
    {
        ...
        public void GetModule()
        {
            try
            {
                DataSet ds = dal_DB.DB_REVIEW_SP(ACTION: "GET_UNFREEZE_REQ_MODULES", SYSTEM: drpSystem.SelectedValue);
        ...
        protected void btnApproveUnFreezeReq_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < lstSystems.Items.Count; i++)
                {
                    dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_UNFREEZE_REQ_APP",
                ...
                dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_UNFREEZE_STATUS",
                        MODULEID: hdnModuleid.Value,
                        REVIEW: false,
                        REASON: txtReason.Text
                        );

                SendEmail("Approved Unfreeze Request", txtReason.Text);

                string MSG = "Un-Freeze request has been approved for " + hdnModulename.Value + " module.";
        ...
        protected void btnDisapproveUnFreezeReq_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < lstSystems.Items.Count; i++)
                {
                    dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_UNFREEZE_REQ_DisApp",
                        MODULEID: hdnModuleid.Value,
                        SYSTEM: (lstSystems.Items[i].FindControl("lblSystemName") as Label).Text,
                        REVIEW: false,
                        REASON: txtReason.Text
                ...
                SendEmail("Disapproved Unfreeze Request", txtReason.Text);

                string MSG = "Un-Freeze request has been disapproved for " + hdnModulename.Value + " module.";
