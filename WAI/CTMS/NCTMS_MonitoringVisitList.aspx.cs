...
namespace CTMS
{
    public partial class NCTMS_MonitoringVisitList : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void GETDATA()
        {
            try
            {
                DataSet ds = dal.CTMS_DATA_SP(ACTION: "GET_VISIT_REPORT_DATA",
        ...
        protected void grdVisits_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "PrintFU")
                {
                    Response.Redirect("NCTMS_FU_LETTER.aspx?SVID=" + SVID + "&SITEID=" + SITEID + "&VISITID=" + VISITID + "&VISIT=" + VISIT_NAME + "&REPORTTYPE=FU_REPORT");
                }
                else if (e.CommandName == "PrintCL")
                {
                    Response.Redirect("NCTMS_FU_LETTER.aspx?SVID=" + SVID + "&SITEID=" + SITEID + "&VISITID=" + VISITID + "&VISIT=" + VISIT_NAME + "&REPORTTYPE=CL_REPORT");
                }
                else if (e.CommandName == "PrintCOL")
                {
                    Response.Redirect("NCTMS_FU_LETTER.aspx?SVID=" + SVID + "&SITEID=" + SITEID + "&VISITID=" + VISITID + "&VISIT=" + VISIT_NAME + "&REPORTTYPE=COL_REPORT");
                }
                else
                {
                    Response.Redirect("NCTMS_VISIT_REPORT.aspx?SVID=" + SVID + "&SITEID=" + SITEID + "&VISITID=" + VISITID + "&VISIT=" + VISIT_NAME);
                }
            }
            catch (Exception ex)
            {
