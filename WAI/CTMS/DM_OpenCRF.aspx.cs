...
namespace CTMS
{
    public partial class DM_OpenCRF : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    if (!string.IsNullOrEmpty(drpSubID.SelectedValue) && drpSubID.SelectedValue != "0")
                    {
                        FillVisit();
                        OpenCRF();
                        CHECK_UNSCHEDULEDVISIT();
                        lbtnRefresh.Visible = true;
                    }
                    else
                    {
                        lbtnAddUnscheduled.Visible = false;
        ...
        protected void Grd_OpenCRF_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "GOTOPAGE")
                {
                    ...
                    {
                        Response.Redirect("DM_DataEntry_MultipleData.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID);
                    }
                    else
                    {
                        Response.Redirect("DM_DataEntry.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID);
                    }
                }
        ...
        protected void lbtnAddUnscheduled_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string URL = "DM_OpenCRF.aspx?SUBJID=" + drpSubID.SelectedValue + "";
                Response.Write("<script> alert('Unscheduled visit added successfully.');window.location='" + URL + "'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void lbtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Write("<script> alert('Rules successfully refreshed.');window.location='DM_OpenCRF.aspx?VISITNUM=" + drpVisit.SelectedValue + "&SUBJID=" + drpSubID.SelectedValue + "'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void CHECK_UNSCHEDULEDVISIT()
        {
            try 
            {
                DataSet ds = dal_DM.DM_UNSC_VISIT_SP(ACTION: "Check_Unscheduled");
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0) 
                {
                    lbtnAddUnscheduled.Visible = true;
                }
                else
                {
                    lbtnAddUnscheduled.Visible = false;
