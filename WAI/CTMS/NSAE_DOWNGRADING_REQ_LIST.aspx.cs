...
namespace CTMS
{
    public partial class NSAE_DOWNGRADING_REQ_LIST : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!this.IsPostBack)
                {
                    DataSet ds = dal_SAE.SAE_DELETED_SP(ACTION: "GET_DOWNGRADING_REQ_SAE_DETAILS",
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        lblDOWNREQBy.Text = ds.Tables[0].Rows[0]["REQ_BYNAME"].ToString();
                        lblDOWNREQDTUTC.Text = ds.Tables[0].Rows[0]["REQ_CAL_TZDAT"].ToString();
                        lblDOWNREQDTServer.Text = ds.Tables[0].Rows[0]["REQ_CAL_DAT"].ToString();
                        lblDOWNREQReason.Text = ds.Tables[0].Rows[0]["REQ_REASON"].ToString();
                    }
        ...
        protected void GET_REQUEST_LOGS()
        {
            try
            {
                DataSet ds = dal_SAE.SAE_DELETED_SP(ACTION: "GET_REQUEST_LOGS",
                    SAEID: lblSAEID.Text,
        ...
        protected void repeatData_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    DataSet ds = dal_SAE.SAE_DELETED_SP(
                            ACTION: "GET_DOWNGRADING_REQ_SAE_RECORD",
                            SAEID: lblSAEID.Text,
        ...
        protected void btnApproveReq_Click(object sender, EventArgs e)
        {
            try
            {
                SaveDataStatus("Delete");

                dal_SAE.SAE_DELETED_SP(
                    ACTION: "UPDATE_DOWNGRADING_REQ_APPR_ACTION",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text,
                    STATUS: lblStatus.Text
                    );

                string MESSAGE = "Downgrading SAE request has been approved Successfully. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), "Approved", "Downgrading SAE Request (Approve/Disapprove)");

                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('" + MESSAGE.ToString() + "'); window.location='NSAE_DOWNGRADING_REQ.aspx';", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void INSERT_SAE_STATUS_LOGS(string STATUS)
        {
            try
            {
        ...
        protected void btnDisApproveReq_Click(object sender, EventArgs e)
        {
            ...
            {
                dal_SAE.SAE_DELETED_SP(
                    ACTION: "UPDATE_DOWNGRADING_REQ_DISSAPPR_ACTION",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text,
                    STATUS: lblStatus.Text
                    );

                string MESSAGE = "Downgrading SAE request has been dissapproved. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), "Disapproved", "Downgrading SAE Request (Approve/Disapprove)");
