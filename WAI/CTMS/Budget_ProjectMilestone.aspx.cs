...
namespace CTMS
{
    public partial class Budget_ProjectMilestone : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void bind_Dept()
        {
            try
            {
                DataSet ds = dal.Budget_SP(Action: "get_PROJECT_Dept_TOM_Milestone", Project_Id: Session["PROJECTID"].ToString());
                if (ds.Tables[0].Rows.Count > 0)
                {
        ...
        protected void gvMainTask_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.Budget_SP(Action: "get_project_Milestone", Project_Id: Session["PROJECTID"].ToString(), Task_ID: Task_ID);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
        ...
        private void GETDATA()
        {
            try
            {
                DataSet ds = dal.Budget_SP(Action: "get_PROJECT_Task_Milestone", Dept_Id: ddlDepartment.SelectedValue);
