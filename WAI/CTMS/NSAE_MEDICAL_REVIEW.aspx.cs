...
namespace CTMS
{
    public partial class NSAE_MEDICAL_REVIEW : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!this.IsPostBack)
                {
                    if(Request.QueryString["ACTION"].ToString() == "Pending")
                    {
                        Heading.Text = "SAE to be Review";
                    }
                    else
                    {
                        Heading.Text = "SAE Reviewed";
                    }
                    FillINV();
                    GetData();
                }
        ...
        public void GetData()
        {
            try
            {
                ...
                if (Request.QueryString["ACTION"].ToString() == "Done")
                {
                    ds = dal_SAE.SAE_STATUS_LIST_SP(ACTION: "GET_SAE_MR_DONE",
                        SUBJID: drpSubID.SelectedValue,
                        INVID: drpInvID.SelectedValue
                        );
                }
                else if (Request.QueryString["ACTION"].ToString() == "Pending")
                {
                    ds = dal_SAE.SAE_STATUS_LIST_SP(ACTION: "GET_SAE_MR_PENDING",
                        SUBJID: drpSubID.SelectedValue,
                        INVID: drpInvID.SelectedValue
                        );
                }
        ...
        protected void grdSAELog_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "EditSAE")
                {
                    ...
                    Response.Redirect("NSAE_MODULE_DATA_LOGS_MR.aspx?INVID=" + INVID + "&SUBJID=" + SUBJID + "&SAE=" + SAE + "&STATUS=" + STATUS + "&SAEID=" + SAEID + "&ACTION=" + Request.QueryString["ACTION"].ToString(), false);
                }
            }
            catch (Exception ex)
            {
