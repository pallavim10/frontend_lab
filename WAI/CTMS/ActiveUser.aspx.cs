    ...
    public partial class ActiveUser : System.Web.UI.Page
    {
        ...
        protected void Btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                User_ID: hnfUserID.Value, ENTEREDBY: Session["User_ID"].ToString());

                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('User Activated successfully'); window.location='ActiveUser.aspx';", true);
            }
        ...
        protected void SendEmail(string User_Name, string Email_ID)
        {
            try
            {
                ...
                String ACTDAT = dr["ACTDAT"].ToString();
                con.Close();
                EmailAdd = Email_ID;                
                ...
                con.Open();
                //Get Data
                cmd8.CommandText = "PROJDETAILS_SP";
                cmd8.CommandType = CommandType.StoredProcedure;
                cmd8.Parameters.AddWithValue("@Action", "Get_Project_Modules");
                adp = new SqlDataAdapter(cmd8);
                adp.Fill(dsModule);
                cmd8.Dispose();
                con.Close();

                string eSource = dsModule.Tables[0].Rows[0]["eSource"].ToString();
                string MODULES = dsModule.Tables[0].Rows[0]["MODULES"].ToString();

                
                if (ACTDAT == "" || ACTDAT == null)
                {
                    if (UserType == "Investigator" || UserType == "Investigator Team Member")
                    {
                        //send mail with user role
                        E_Sub = "Protocol Id - " + Drp_Project_Name.SelectedItem.Text.ToString() + " : New User ID Created";

                        if (eSource == "True")
                        {
                            E_Body = "Hi " + Drp_User.SelectedItem.Text + ", </br>Your Login Details: </br>URL for " + MODULES + " : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/Auth.aspx") + " </br> URL for eSource : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/eSource/Auth.aspx") + " </br> User Role:" + Drp_User_Group.SelectedItem.Text + "";
                        }
                        else
                        {
                            E_Body = "Hi " + Drp_User.SelectedItem.Text + ", </br>Your Login Details: </br>URL for " + MODULES + " : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/Auth.aspx") + " </br> User Role:" + Drp_User_Group.SelectedItem.Text + "";

                        }

                        commFun.Email_Users(EmailAdd, CCEmailAddress, E_Sub, E_Body);

                        //send mail with user role and user id
                        EmailAdd = Email_ID;
                        ...
                        if (eSource == "True")
                        {
                            E_Body = "Hi " + Drp_User.SelectedItem.Text + ", </br>Your Login Details: </br>URL for " + MODULES + " : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/Auth.aspx") + " </br> URL for eSource : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/eSource/Auth.aspx") + " </br> Your User ID: " + UID + " </br>User Role:" + Drp_User_Group.SelectedItem.Text + "";
                        }
                        else
                        {
                            E_Body = "Hi " + Drp_User.SelectedItem.Text + ", </br>Your Login Details: </br>URL for " + MODULES + " : " + HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/Auth.aspx") + " </br> Your User ID: " + UID + " </br>User Role:" + Drp_User_Group.SelectedItem.Text + "";
                        }

                        commFun.Email_Users(EmailAdd, "", E_Sub, E_Body);
                    }
                    else
                    {
                        //send mail with user role
                ...
                else
                {
                    ...
                    {
                        //send mail with user role
                        E_Sub = "Protocol Id - " + Drp_Project_Name.SelectedItem.Text.ToString() + " : User Re-Activated";
                ...
                //Send PWD in Email
                EmailAdd = Email_ID;
                E_Sub = "Protocol Id - " + Drp_Project_Name.SelectedItem.Text.ToString() + " : Password";
                E_Body = "Hi " + Drp_User.SelectedItem.Text + ", </br>Your Login Password: " + PWD + " ";
                commFun.Email_Users(EmailAdd, "", E_Sub, E_Body);
            }
            catch (Exception ex)
            {
