...
namespace CTMS
{
    public partial class eTMF_Snapshot_SPONSOR : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void BIND_SECTIONS()
        {
            try
            {
                ...
                if (Request.QueryString["SNAPID"] != null)
                {
                    ds = dal.eTMF_Snapshot_SP(ACTION: "GET_SECTIONS",
        ...
        private void BIND_ARTIFACT()
        {
            try
            {
                ...
                if (Request.QueryString["SNAPID"] != null)
                {
                    ds = dal.eTMF_Snapshot_SP(ACTION: "GET_ARTIFACTS",
        ...
        protected void gvSubFolder_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_Snapshot_SP(ACTION: "GET_ARTIFACTS", SectionId: SubFolder_ID, ZoneID: ddlZone.SelectedValue, SnapId: Request.QueryString["SNAPID"].ToString());
                    gvArtifact.DataSource = ds.Tables[0];
                    gvArtifact.DataBind();
        ...
        protected void gvArtifact_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_Snapshot_SP(ACTION: "get_BD_Docs_FileStructure_SPONSOR",
                    RefNo: MainRefNo,
                    DocTypeId: DocTypeId,
                    SnapId: Request.QueryString["SNAPID"].ToString()
                    );
        ...
        private void ShowAs()
        {
            try
            {
                ...
                {
                    gvArtifact.Visible = false;
                    gvFiles_List.Visible = true;
                }
                else
                {
                    gvArtifact.Visible = true;
