...
namespace CTMS
{
    public partial class Comm_Delete : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void Get_Delete()
        {
            try
            {
                DataSet ds = dal.Communication_SP(Action: "Get_Delete", UserID: Session["User_ID"].ToString(), PROJECTID: Session["PROJECTID"].ToString(), Reference: txtAutoCompText.Text);
        ...
        protected void repeatDelete_ItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                {
                    dal.Communication_SP(Action: "Permanent_Delete", ID: ID);
                }
