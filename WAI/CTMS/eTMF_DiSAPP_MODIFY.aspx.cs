...
namespace CTMS
{
    public partial class eTMF_DiSAPP_MODIFY : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    REUPLOAD_DOCUMENT(DocTypeId, ZoneId, SectionId, ArtifactId);
                }
                else
                {
                ...
                Response.Write("<script> alert('Document Updated successfully.');window.location='eTMF_QC_DISAPPROVED.aspx';</script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
        ...
        private void REUPLOAD_DOCUMENT(string DocTypeId, string ZoneId, string SectionId, string ArtifactId)
        {
            try
            {
                string folderPath = Server.MapPath("~/eTMF_Docs/");
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    Size = FileUpload1.PostedFile.ContentLength.ToString();
                    extension = Path.GetExtension(UploadFileName);

                    string uploadDirectory = Server.MapPath("~/eTMF_Docs/");

                    folderPath = Path.Combine(uploadDirectory, UploadFileName);

                    txtFileName1.Text = UploadFileName;
                }

                DataSet ds = dal.eTMF_QC_SP
                        (
                            ACTION: "REUPLOAD_DOCUMENT_QC",
                            CountryID: drpCountry.SelectedValue,
                            SiteID: drpSites.SelectedValue,
                            DocID: drpDocument.SelectedValue,
                            DocName: drpDocument.SelectedItem.Text,
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("eTMF_QC_DISAPPROVED.aspx");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = dal.eTMF_QC_SP(ACTION: "DELETE_DOCUMENT_QC",
                ID: Request.QueryString["ID"].ToString(),
                UploadBy: Session["User_Id"].ToString(),
                NOTE: txtReason.Text
                );

                Response.Write("<script> alert('Document Deleted successfully.');window.location.href='eTMF_QC_DISAPPROVED.aspx';</script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
        ...
        protected void ddltakeaction_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddltakeaction.SelectedItem.Text == "Modify/Reupload")
            {
                DivModifyDoc.Visible = true;
                DivDELETEDOC.Visible = false;

            }
            else if (ddltakeaction.SelectedItem.Text == "Delete")
            {
                DivDELETEDOC.Visible = true;
                DivModifyDoc.Visible = false;
            }
            else
            {
                DivDELETEDOC.Visible = false;
