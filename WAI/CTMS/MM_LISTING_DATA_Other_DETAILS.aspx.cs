...
namespace CTMS
{
    public partial class MM_LISTING_DATA_Other_DETAILS : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (lblHeader.Text == "Lab Results Tabular")
                    {
                        DataSet ds1 = dal_MM.MM_LAB_SP(
                        ACTION: "MM_LAB_RESULT_TABULAR_ALL",
                        SUBJID: hdnSUBJID.Value
                        );

                        if (ds1.Tables[0].Rows.Count > 0)
                        {
                            gridData_Tran.DataSource = ds1;
