...
namespace CTMS
{
    public partial class DB_FROZEN_MODULES : System.Web.UI.Page
    {
        ...
        public void GetModule()
        {
            try
            {
                DataSet ds = dal_DB.DB_REVIEW_SP(ACTION: "GET_FROZEN_MODULES", SYSTEM: drpSystem.SelectedValue);
        ...
        protected void lbtnUnFreezeReq_Click(object sender, EventArgs e)
        {
            try
            {
                txtReason.Attributes.Add("MaxLength", "500");
                ...
                DataSet ds = dal_DB.DB_REVIEW_SP(ACTION: "GET_SYSTEMS_FOR_GEN_UNFREEZE_REQUEST",
                    MODULEID: MODULEID
                    );
                var migrationValue = ds.Tables[1].Rows[0]["MIGRATION"];
                if (ds.Tables[1].Rows.Count > 0 && migrationValue.ToString() == "False")
                {
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Please unlock the DB status to proceed with the unfreeze request.')", true);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnSubmitUnFreezeReq_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < lstSystems.Items.Count; i++)
                {
                    dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_UNFREEZE_REQ",
                ...
                SendEmail("Generate Unfreeze Request", txtReason.Text);

                string MSG = "Un-Freeze request has been generated for " + hdnModulename.Value + " module.";
