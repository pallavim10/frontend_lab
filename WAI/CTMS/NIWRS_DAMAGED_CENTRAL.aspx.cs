...
namespace CTMS
{
    public partial class NIWRS_DAMAGED_CENTRAL : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            TxtDestoryComm.Attributes.Add("MaxLength", "200");
            txtReturnComm.Attributes.Add("MaxLength", "200");
            try
            {
                if (!IsPostBack)
                {
        ...
        private void GET_KITS()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_CENTRAL_SP(ACTION: "GET_DAMAGED_CENTRAL", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        private void SEND_MAIL_UNQUARANTINE(string KITNO, string Location, string ORDERID)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_DAMAGED_CENTRAL - UnQuarantine");
            ...
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    if (Chek_UnQuarantine.Checked)
                    {
                        ...
                        SEND_MAIL_UNQUARANTINE(KITNO, Location, ORDERID);
                    }
                }
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Reverted to Use Successfully.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                hfKITS.Value = "";
        ...
        private void ExportSingleSheet()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_CENTRAL_SP(ACTION: "GET_DAMAGED_CENTRAL_EXPORT", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        protected void Destory_Click(object sender, EventArgs e)
        {
            try
            {
                string selectedKitsCsv = hfKITS.Value;
        ...
        protected void btnSubmitDestory_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    string TABLENAME = ((Label)gvKits.Rows[i].FindControl("TABLENAME")).Text;
                    string ORDERID = ((Label)gvKits.Rows[i].FindControl("ORDERID")).Text;
                    CheckBox Chek_UnDestory = (CheckBox)gvKits.Rows[i].FindControl("Chek_UnDestory");

                    string selectedKitsCsv = hfKITS.Value;
                    if (selectedKitsCsv.Contains(KITNO))
                    {
                        Chek_UnDestory.Checked = true;
                    }

                    if (Chek_UnDestory.Checked)
                    {
                        dal_IWRS.IWRS_KITS_CENTRAL_SP(
                                ACTION: "DESTORYED_CENTRAL_KIT",
                                KITNO: KITNO,
                                TREAT_GRP: Location,
                                DESTROYCOMM: TxtDestoryComm.Text,
                                TABLENAME: TABLENAME
                            );

                        SEND_MAIL_DESTORYED(KITNO, Location, ORDERID);
                    }
                }
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Destory Kit Successfully.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                hfKITS.Value = "";
            }
            catch (Exception ex)
            {
        ...
        protected void btnCancelDestory_Click(object sender, EventArgs e)
        {
            try
            {
                Page.Response.Redirect(Page.Request.Url.ToString(), true);
        ...
        private void SEND_MAIL_DESTORYED(string KITNO, string Location, string ORDERID)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_DAMAGED_CENTRAL - Destroy");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[Location]", Location);
                    BODY = BODY.Replace("[ORDERID]", ORDERID);
                    BODY = BODY.Replace("[RETURNAT]", Location);
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        protected void btnSubmitReturn_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    string TABLENAME = ((Label)gvKits.Rows[i].FindControl("TABLENAME")).Text;
                    string ORDERID = ((Label)gvKits.Rows[i].FindControl("ORDERID")).Text;
                    CheckBox Chek_UnReturn = (CheckBox)gvKits.Rows[i].FindControl("Chek_UnReturn");

                    string selectedKitsCsv = hfKITS.Value;
                    if (selectedKitsCsv.Contains(KITNO))
                    {
                        Chek_UnReturn.Checked = true;
                    }


                    if (Chek_UnReturn.Checked)
                    {
                        dal_IWRS.IWRS_KITS_CENTRAL_SP(
                                ACTION: "RETURNED_CENTRAL_KIT",
                                KITNO: KITNO,
                                TREAT_GRP: Location,
                                RETURNEDCOMM: txtReturnComm.Text,
                                TABLENAME: TABLENAME
                            );

                        SEND_MAIL_RETURNED(KITNO, Location, ORDERID);
                    }
                }
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Returned Kit Successfully.'); window.location.href = '" + Request.RawUrl.ToString() + "'; ", true);
                hfKITS.Value = "";
            }
            catch (Exception ex)
            {
        ...
        private void SEND_MAIL_RETURNED(string KITNO, string Location, string ORDERID)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_DAMAGED_CENTRAL - Return");
