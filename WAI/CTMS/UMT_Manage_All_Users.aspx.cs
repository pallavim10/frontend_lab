    ...
    public partial class UMT_Manage_All_Users : System.Web.UI.Page
    {
        ...
        private void USER_ACTIVATION_MAIL_FIRST(string User_EmailID, string USERNAME, string STUDYROLE, string USERID)
        {
            try
            {
                DataSet ds = new DataSet();
                DataSet dsEmail = new DataSet();
                string EMAILIDS = "", CCEMAILIDS = "", BCCEMAILIDS = "", SUBJECT = "", BODY = "", ACTIONS = "", LIST_SYSTEM = "", htmlTableBody ="", URL = "",  ESOURCE_URL = "";
                ...
                bool isEsource = false;

                foreach (DataRow dr in ds1.Tables[0].Rows)
                {

                    if (dr["System Name"].ToString() == "eSource")
                    {
                        isEsource = true;
                    }
                }
                URL = HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/Login.aspx");
                ESOURCE_URL = HttpContext.Current.Request.Url.AbsoluteUri.ToString().Replace(Request.RawUrl.ToString(), "/eSource/Auth.aspx");
                SUBJECT = "User Activation";
                if (isEsource)
                {
                    dsEmail = dal.UMT_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", Email_Code: "USER_ACTIVATION_MAIL_FIRST_ESOURCE");
                }
                else
                {
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[ESOURCE_URL]", ESOURCE_URL);
        ...
        protected void lblUserDetailsExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Multiple_Export_Excel.ToExcel(dsExport, header, Page.Response);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
