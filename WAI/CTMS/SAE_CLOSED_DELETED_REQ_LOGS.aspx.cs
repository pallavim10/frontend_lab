...
namespace CTMS
{
    public partial class SAE_CLOSED_DELETED_REQ_LOGS : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_SAEID()
        {
            try
            {
                DataSet ds = dal_SAE.SAE_REPORT_SP(ACTION: "GET_SAEIDS_WITH_IIC", SUBJID: ddlSUBJID.SelectedValue);
                if (ds.Tables[0].Rows.Count > 0)
                {
        ...
        private void GET_SAE_EVENT_LOGS()
        {
            try
            {
                DataSet ds = dal_SAE.SAE_REPORT_SP(ACTION: "GET_CLOSED_DELETE_REQUEST_LOGS",
        ...
        private void ExportSingleSheet()
        {
            try
            {
                ...
                string xlname = Session["PROJECTIDTEXT"].ToString() + "_Closed & Downgraded Request Logs_" + DateTime.Now.ToString("yyyyMMdd") + ".xls";
