...
namespace CTMS
{
    public partial class DM_Query_Reports_Unblinding : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Session["QUERY_TYPE_UNBLIND"] != null)
                    {
                        if (drpQueryType.Items.Contains(new ListItem(Session["QUERY_TYPE_UNBLIND"].ToString())))
                        {
                            drpQueryType.SelectedValue = Session["QUERY_TYPE_UNBLIND"].ToString();
                        }
                    }

                    Session["QUERY_TYPE_UNBLIND"] = drpQueryType.SelectedValue;
        ...
        public void FillINV()
        {
            ...
            if (Session["QUERY_INVID_UNBLIND"] != null)
            {
                if (drpSite.Items.Contains(new ListItem(Session["QUERY_INVID_UNBLIND"].ToString())))
                {
                    drpSite.SelectedValue = Session["QUERY_INVID_UNBLIND"].ToString();
                }
            }

            Session["QUERY_INVID_UNBLIND"] = drpSite.SelectedValue;
        ...
        private void FillSubject()
        {
            try
            {
                ...
                if (Session["QUERY_SUBJID_UNBLIND"] != null)
                {
                    if (drpPatient.Items.FindByValue(Session["QUERY_SUBJID_UNBLIND"].ToString()) != null)
                    {
                        drpPatient.SelectedValue = Session["QUERY_SUBJID_UNBLIND"].ToString();
                    }
                }

                Session["QUERY_SUBJID_UNBLIND"] = drpPatient.SelectedValue;
            }
            catch (Exception ex)
            {
        ...
        private void FillVisit()
        {
            try
            {
                ...
                if (Session["QUERY_VISITID_UNBLIND"] != null)
                {
                    if (drpVisit.Items.FindByValue(Session["QUERY_VISITID_UNBLIND"].ToString()) != null)
                    {
                        drpVisit.SelectedValue = Session["QUERY_VISITID_UNBLIND"].ToString();
                    }
                }

                Session["QUERY_VISITID_UNBLIND"] = drpVisit.SelectedValue;
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        private void FillModule()
        {
            try
            {
                ...
                if (Session["QUERY_MODULEID_UNBLIND"] != null)
                {
                    if (drpModule.Items.FindByValue(Session["QUERY_MODULEID_UNBLIND"].ToString()) != null)
                    {
                        drpModule.SelectedValue = Session["QUERY_MODULEID_UNBLIND"].ToString();
                    }
                }

                Session["QUERY_MODULEID_UNBLIND"] = drpModule.SelectedValue;
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        private void FillFielde()
        {
            try
            {
                ...
                if (Session["QUERY_FIELDID_UNBLIND"] != null)
                {
                    if (drpField.Items.FindByValue(Session["QUERY_FIELDID_UNBLIND"].ToString()) != null)
                    {
                        drpField.SelectedValue = Session["QUERY_FIELDID_UNBLIND"].ToString();
                    }
                }

                Session["QUERY_FIELDID_UNBLIND"] = drpField.SelectedValue;
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        private void FillQueryStatus()
        {
            try
            {
                ...
                if (Session["QUERY_STATUS_UNBLIND"] != null)
                {
                    if (drpQueryStatus.Items.FindByValue(Session["QUERY_STATUS_UNBLIND"].ToString()) != null)
                    {
                        drpQueryStatus.SelectedValue = Session["QUERY_STATUS_UNBLIND"].ToString();
                    }
                }

                Session["QUERY_STATUS_UNBLIND"] = drpQueryStatus.SelectedValue;
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        private void GETDATA()
        {
            try
            {
                ...
                ACTION: "GET_UNBLINDED_QUERY_REPORT",
        ...
        protected void grdQueryDetailReports_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                Session["QUERY_URL"] = "DM_Query_Reports_Unblinding.aspx";
                Response.Redirect(URL);
            }
            catch (Exception ex)
            {
        ...
        private void ExportSingleSheet()
        {
            ...
            {
                DataSet ds = dal_DM.DM_QUERY_REPORT_SP(
                   ACTION: "GET_UNBLINDED_QUERY_REPORT_EXPORT",
                   INVID: drpSite.SelectedValue,
                   SUBJID: drpPatient.SelectedValue,
                   VISITNUM: drpVisit.SelectedValue,
                   MODULEID: drpModule.SelectedValue,
                ...
                string xlname = Session["PROJECTIDTEXT"].ToString() + "_DM Unblinded Query Report_" + DateTime.Now.ToString("yyyyMMdd") + ".xls";
