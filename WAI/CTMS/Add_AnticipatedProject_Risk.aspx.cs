...
namespace CTMS
{
    public partial class Add_AnticipatedProject_Risk : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Btn_Get_Fun_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = dal.getsetRisk_SP(Action: "Get_Anticipated_Risk_New", Project_ID: Drp_Project_Name.SelectedValue);
            ...
            {                      
                ...
                {               
                    ...
                    {                 
                        ...
                        DataSet ds = dal.getsetRisk_SP(Action: "Insert_ProjectRisk", Project_ID: Drp_Project_Name.SelectedValue, RISK_ID: Risk_ID, ENTEREDBY: Session["User_ID"].ToString());
                    }
                }
                Response.Write("<script> alert('Record Updated successfully.');window.location='Add_AnticipatedProject_Risk.aspx'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
