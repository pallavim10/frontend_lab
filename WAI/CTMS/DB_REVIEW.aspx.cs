...
namespace CTMS
{
    public partial class DB_REVIEW : System.Web.UI.Page
    {
        ...
        protected void SHOW_SEND_BACK_DESGINER()
        {
            try
            {
                DataSet ds = dal_DB.DB_REVIEW_SP(ACTION: "GET_MODULE_STATUS",
                ...
                {
                    if (ds.Tables[0].Rows[0]["REVIEWED_COUNT"].ToString() != "0")
                    {
                        btnBackToDesigner.Visible = false;
                    }
                    else if (ds.Tables[0].Rows[0]["REVIEWED_COUNT"].ToString() == "0")
                    {
                        btnBackToDesigner.Visible = true;
                    }

                    if (ds.Tables[0].Rows[0]["UNREVIEW_REQ_GEN_COUNT"].ToString() != "0")
                    {
                        lblReview.Text = "Un-Review	Request Generated.";
        ...
        protected void drpSystem_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                gridreviewlogs.DataSource = null;
                gridreviewlogs.DataBind();
                divReviewLogs.Visible = false;
        ...
        public void GetModule()
        {
            try
            {
                DataSet ds = dal_DB.DB_REVIEW_SP(ACTION: "GET_MODULENAME_BY_SYSTEM_FOR_REVIEW", SYSTEM: drpSystem.SelectedValue);
        ...
        protected void btnShowReview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SHOW_SEND_BACK_DESGINER();
                GET_MODULE_REVIEW_LOGS();
                GET_OnSubmit_Crit();
                GET_OnChange_Crit();
                GET_MODULE_CRITs();
        ...
        protected void grd_Data_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds_DATE = dal_DB.DB_REVIEW_SP(
                               ACTION: "GET_REVEIW_LOG_DATE",
                               MODULEID: drpModule.SelectedValue,
                               SYSTEM: drpSystem.SelectedItem.Text
                               );


                    if (ds_DATE.Tables.Count > 0)
                    {
                        bool highlightRow = false;
                        for (int i = 0; i < ds_DATE.Tables[0].Rows.Count; i++)
                        {
                            // Directly get the values as strings, handling null values
                            string LastChangeDate = ds_DATE.Tables[0].Rows[i]["LastChangeDate"] != DBNull.Value
                                                    ? ds_DATE.Tables[0].Rows[i]["LastChangeDate"].ToString()
                                                    : null;

                            string PreviousChangeDate = ds_DATE.Tables[0].Rows[i]["PreviousChangeDate"] != DBNull.Value
                                                        ? ds_DATE.Tables[0].Rows[i]["PreviousChangeDate"].ToString()
                                                        : null;

                            if (PreviousChangeDate != null)
                            {
                                DataSet ds_LOG = dal_DB.DB_REVIEW_SP(
                                   ACTION: "GET_REVEIW_LOGS",
                                   FIELDID: ID,
                                ...
                                // Check if there are logs, and highlight the row if there are results
                                if (ds_LOG.Tables.Count > 0 && ds_LOG.Tables[0].Rows.Count > 0)
                                {
                                    highlightRow = true;
                                }

                                // Apply CSS class based on whether there are logs
                                if (highlightRow)
                                {
                                    e.Row.CssClass = "highlight-border"; // Highlight the row
                                }
                                else
                                {
                                    e.Row.CssClass = ""; // Optionally reset the class
                                }
                            }
                        }
                    }
                    ...
                    DataSet ds1 = dal_DB.DB_REVIEW_SP(
        ...
        private void GET_MODULE_REVIEW_LOGS()
        {
            try
            {
                DataSet ds = dal_DB.DB_REVIEW_SP(
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    gridreviewlogs.DataSource = ds;
                    gridreviewlogs.DataBind();
                    divReviewLogs.Visible = true;
                }
                else
                {
                    gridreviewlogs.DataSource = null;
        ...
        protected void GET_MODULE_CRITs()
        {
            try
            {
                DataSet ds = dal_DB.DB_SETUP_CRITs_SP(ACTION: "GET_MODULE_CRITs", MODULEID: drpModule.SelectedValue);
        ...
        private void GET_LAB_DEFAULTS_DATA()
        {
            try
            {
                DataSet ds = dal_DB.DB_LAB_REFERENCE_SP(ACTION: "GET_DEFAULT_MAPPING", MODULEID: drpModule.SelectedValue);
        ...
        protected void btnSubmitReview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_MODULE_REVIEW",
                        MODULEID: drpModule.SelectedValue,
                ...
                SendEmail("Reviewed", txtReason.Text);

                string MSG = lblModuleName.Text + " has been reviewed for " + Systems + " System.";
        ...
        protected void ClearReviewReason()
        {
            try
            {
        ...
        protected void btnBackToDesigner_Click(object sender, EventArgs e)
        {
            try
            {
                txtBackToDesinerComment.Attributes.Add("MaxLength", "500");
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    lblBackToDesigner.InnerText = drpModule.SelectedItem.Text + " module is available at below systems.";

                    lstBackToDesigner.DataSource = ds.Tables[0];
                    lstBackToDesigner.DataBind();
                }
                else
                {
                    lstBackToDesigner.DataSource = null;
                    lstBackToDesigner.DataBind();
                }
        ...
        protected void btnSubmitUnReview_Click(object sender, EventArgs e)
        {
            try
            {
                string Systems = "";

                for (int i = 0; i < lstBackToDesigner.Items.Count; i++)
                {
                    dal_DB.DB_REVIEW_SP(ACTION: "UPDATE_SEND_BACK_TO_DESIGNER_FROM_REVIEWER",
                        MODULEID: drpModule.SelectedValue,
                        SYSTEM: (lstBackToDesigner.Items[i].FindControl("lblSystemName") as Label).Text,
                        REVIEW: false,
                        REASON: txtBackToDesinerComment.Text
                        );

                    if (Systems == "")
                    {
                        Systems = (lstBackToDesigner.Items[i].FindControl("lblSystemName") as Label).Text;
                    }
                    else
                    {
                        Systems += ", " + (lstBackToDesigner.Items[i].FindControl("lblSystemName") as Label).Text;
                ...
                SendEmail("Send Back To Designer From Reviewer", txtBackToDesinerComment.Text);

                string MSG = drpModule.SelectedItem.Text + " has been sent back to designer for " + Systems + " System.";
        ...
        protected void btnCancelUnReview_Click(object sender, EventArgs e)
        {
            try
            {
                ClearSendtodesignerReason();
        ...
        protected void ClearSendtodesignerReason()
        {
            try
            {
                txtBackToDesinerComment.Text = string.Empty;
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
