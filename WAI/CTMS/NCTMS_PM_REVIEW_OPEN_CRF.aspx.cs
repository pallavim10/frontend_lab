...
namespace CTMS
{
    public partial class NCTMS_PM_REVIEW_OPEN_CRF : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        public void FILL_MODULES()
        {
            try
            {
                ...
                ACTION: "GET_MODULES_AGAINST_VISIT_NOM_PM",
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    btnPMFinal.Visible = true;
                    btnSendToSponsor.Visible = true;
                    btnReturnToCRA.Visible = true;
                }
                else
                {
                    ...
                    btnPMFinal.Visible = false;
                    btnSendToSponsor.Visible = false;
                    btnReturnToCRA.Visible = false;
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (e.CommandName == "GOTOPAGE")
                {
                    ...
                    if ((gvr.FindControl("MULTIPLEYN") as Label).Text == "True")
                    {
                        ...
                        Response.Redirect("NCTMS_DataEntry_MultipleData_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&VISIT_NOM=" + hdnSVID.Value + "&SVID=" + hdnSVID.Value + "&ACTION=PM");
                    }
                    else
                    {
                        Session["BACKTOMAINPAGE"] = Request.RawUrl.ToString();
                        Response.Redirect("NCTMS_DataEntry_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&VISIT_NOM=" + hdnSVID.Value + "&SVID=" + hdnSVID.Value + "&ACTION=PM");
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void btnPMFinal_Click(object sender, EventArgs e)
        {
            try
            {
                InsertUpdateSV("Final", "Final by PM");

                INSERT_COMMENT_PD_DATA();

                SendEmail("Final_by_PM");

                string MESSAGE = hdnVISIT.Value + " with Visit Id: " + hdnSVID.Value + " is submited successfully by PM";

                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('" + MESSAGE.ToString() + "'); window.location='NCTMS_PM_REVIEW_LIST.aspx';", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void btnSendToSponsor_Click(object sender, EventArgs e)
        {
            try
            {
                InsertUpdateSV("For Approval", "Send To Sponsor Review");

                SendEmail("Send_To_Sponsor_Review");

                string MESSAGE = hdnVISIT.Value + " with Visit Id: " + hdnSVID.Value + " is submited successfully by PM and Send to Sponsor Review";
        ...
        protected void btnReturnToCRA_Click(object sender, EventArgs e)
        {
            try
            {
                InsertUpdateSV("In-Progress", "Return To CRA");

                SendEmail("Return_To_CRA");

                string MESSAGE = hdnVISIT.Value + " with Visit Id: " + hdnSVID.Value + " is successfully return to CRA";
        ...
        protected void btnBacktoHomePage_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("NCTMS_PM_REVIEW_LIST.aspx");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
