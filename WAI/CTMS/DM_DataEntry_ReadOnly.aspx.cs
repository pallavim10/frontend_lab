...
namespace CTMS
{
    public partial class DM_DataEntry_ReadOnly : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (VISITID != Request.QueryString["VISITID"].ToString())
                    {
                        ...
                        {
                            Response.Redirect("DM_DataEntry_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID + "&OPENLINK=1&REFERENCE=" + Request.QueryString["REFERENCE"].ToString());
                        }
                        else
                        {
                            Response.Redirect("DM_DataEntry_ReadOnly.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID + "&OPENLINK=1");
                        }
                    }
                }
        ...
        protected void btnBacktoHomePage_Click(object sender, EventArgs e)
        {
            try
            {
                if (Session["QUERY_URL"] != null)
                {
                    Session["BACKTOQUERY_REPORT"] = "1";
                    Response.Redirect(Session["QUERY_URL"].ToString());
                }
                else if (Session["MM_QUERY_URL"] != null)
                {
                    Response.Redirect(Session["MM_QUERY_URL"].ToString());
                }
                else
                {
            ...
            {
                Session.Remove("QUERY_URL");
                Session.Remove("MM_QUERY_URL");
                lblErrorMsg.Text = ex.Message.ToString();
            }
        }
        ...
        protected void lnkPAGENUMDeleted_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect("DM_DataEntry_ReadOnly.aspx?MODULEID=" + Request.QueryString["MODULEID"] + "&MODULENAME=" + Request.QueryString["MODULENAME"] + "&VISITID=" + Request.QueryString["VISITID"] + "&VISIT=" + Request.QueryString["VISIT"] + "&INVID=" + Request.QueryString["INVID"] + "&SUBJID=" + Request.QueryString["SUBJID"] + "&RECID=" + REC_ID.Text + "&DELETED=1");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "window.location.href = 'DM_DataEntry_ReadOnly.aspx?MODULEID=" + Request.QueryString["MODULEID"].ToString() + "&MODULENAME=" + Request.QueryString["MODULENAME"].ToString() + "&VISITID=" + Request.QueryString["VISITID"].ToString() + "&VISIT=" + Request.QueryString["VISIT"].ToString() + "&INVID=" + Request.QueryString["INVID"].ToString() + "&SUBJID=" + Request.QueryString["SUBJID"].ToString() + "';", true);
        }
