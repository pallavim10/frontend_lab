...
namespace CTMS
{
    public partial class Train_Verification : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void bind_Labels()
        {
            try
            {
                DataSet ds = dal.EmpMaster_SP(Action: "SELECT", ID: Request.QueryString["Emp_ID"].ToString());
                lblTrainee.Text = ds.Tables[0].Rows[0]["FirstName"].ToString() + " " + ds.Tables[0].Rows[0]["LastName"].ToString();

                DataSet ds1 = dal.Training_SP(Action: "select_TrainPlan_CTMS", ID: Request.QueryString["Plan_ID"].ToString());
        ...
        private void bind_Questions()
        {
            try
            {
                DataSet ds = dal.Train_Verification_SP(Action: "get_Plan_Ques", Plan_ID: Request.QueryString["Plan_ID"].ToString(), SubSection_IDs: Request.QueryString["SubSec"].ToString());
        ...
        protected void gvSections_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    {
                        ddl.Visible = true;
                        DataSet ds = dal.Train_Verification_SP(Action: "get_ddl", QueNo: ID);
            ...
            try
            {
                ...
                for (int i = 0; i < gvSections.Rows.Count; i++)
                {
                    ...
                    else if (CONTROLTYPE == "RADIOBUTTON")
                    {
                        ...
                        {
                            if (((RadioButton)repeat_rbtn.Items[a].FindControl("rbtn")).Checked == true)
                            {
                                data = ((RadioButton)repeat_rbtn.Items[a].FindControl("rbtn")).Text.ToString();
                            }
                        }
                        if (ANS == data)
                        {
                ...
                else
                {
                    ...
                    for (int i = 0; i < values.Length; i++)
                    {
                        ...
                        dal.Train_Verification_SP(Action: "insert_Read_CTMS", Plan_ID: plan, SubSec_ID: values[i], Emp_ID: emp, Project_ID: Session["PROJECTID"].ToString());
                    }
                    Response.Write("<script language='javascript'> alert('***Congratulations*** \\n You have Scored " + correct + " Out of " + gvSections.Rows.Count + ".'); window.location.href = 'Training_Internal.aspx';</script>");
                }
            }
            catch (Exception ex)
            {
