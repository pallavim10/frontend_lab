...
namespace CTMS
{
    public partial class NSAE_DELETED_SAE_DETAILS : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (!this.IsPostBack)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        lblDeletedDTServer.Text = ds.Tables[0].Rows[0]["DELETE_CAL_TZDAT"].ToString();
                        lblDeleteReason.Text = ds.Tables[0].Rows[0]["DELETEREASON"].ToString();

                        if (ds.Tables[0].Rows[0]["REQ"].ToString() == "True" && ds.Tables[0].Rows[0]["ACTION"].ToString() == "Dissapproved")
                        {
                            btnRestoreSAE.Visible = false;
                        }
                        else
                        {
                            btnRestoreSAE.Visible = true;
                        }
        ...
        protected void btnSubmitRestoreSAE_Click(object sender, EventArgs e)
        {
            try
            {
                dal_SAE.SAE_DELETED_SP(
                    ACTION: "UPDATE_RESTORE_SAE_REQ",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text
                    );

                string MESSAGE = "Restore request has been generated Successfully. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), "", "Generate Restore SAE Request");

                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('" + MESSAGE.ToString() + "'); window.location='NSAE_Deleted_SAE.aspx';", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
