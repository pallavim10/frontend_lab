...
namespace CTMS
{
    public partial class DM_UnLockData : System.Web.UI.Page
    {
        ...
        protected void Btn_Get_Data_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "GET_UNLOCK_DATA",
                ...
                if (ds.Tables[0] != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        Grd_unlock.DataSource = ds.Tables[0];
                        Grd_unlock.DataBind();
        ...
        protected void Grd_unlock_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (MULTIPLEYN.Text == "True")
                    {
                        ...
                        DataSet ds = dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "GET_MULTIPLE_RECORDS_UNLOCK",
                            PVID: lblPVID.Text,
        ...
        protected void Gridchild_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    CheckBox Chkchild_UnLockYN = (CheckBox)e.Row.FindControl("Chkchild_UnLockYN");
                    ...
                    {
                        Chkchild_UnLockYN.Visible = false;
        ...
        protected void Btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < Grd_unlock.Rows.Count; i++)
                {
                    CheckBox Chk_UnLockYN = (CheckBox)Grd_unlock.Rows[i].FindControl("Chk_UnLockYN");

                    Label lblSUBJID = (Label)Grd_unlock.Rows[i].FindControl("lblSUBJID");

                    Label lblVISITNUM = (Label)Grd_unlock.Rows[i].FindControl("lblVISITNUM");

                    Label lblVISIT = (Label)Grd_unlock.Rows[i].FindControl("lblVISIT");

                    Label lblMODULEID = (Label)Grd_unlock.Rows[i].FindControl("lblMODULEID");

                    Label lblMODULENAME = (Label)Grd_unlock.Rows[i].FindControl("lblMODULENAME");

                    Label lblPVID = (Label)Grd_unlock.Rows[i].FindControl("lblPVID");

                    Label MULTIPLEYN = (Label)Grd_unlock.Rows[i].FindControl("MULTIPLEYN");

                    Label lblTableName = (Label)Grd_unlock.Rows[i].FindControl("lblTableName");

                    Label lblRECID = (Label)Grd_unlock.Rows[i].FindControl("lblRECID");

                    Label PAGESTATUS = (Label)Grd_unlock.Rows[i].FindControl("PAGESTATUS");

                    if (PAGESTATUS.Text == "2")
                    {
                        if (Chk_UnLockYN.Checked)
                        {
                            dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "UPDATE_UNLOCK_REQUEST_STATUS",
                                PVID: lblPVID.Text,
                                RECID: lblRECID.Text,
                                INVID: drpInvID.SelectedValue,
                                SUBJID: lblSUBJID.Text,
                    ...
                    else
                    {
                        if (MULTIPLEYN.Text == "True")
                        {
                            GridView Gridchild = (GridView)Grd_unlock.Rows[i].FindControl("Gridchild");

                            for (int j = 0; j < Gridchild.Rows.Count; j++)
                            {
                                CheckBox Chkchild_UnLockYN = (CheckBox)Gridchild.Rows[j].FindControl("Chkchild_UnLockYN");

                                if (Chkchild_UnLockYN.Checked)
                                {
                                    dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "UPDATE_UNLOCK_REQUEST_STATUS",
                                    PVID: lblPVID.Text,
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('UnLock Request Generated Successfully'); window.location.href='DM_UnLockData.aspx'", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
        ...
        protected void Chk_Select_All_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (Chk_Select_All.Checked)
                {
                    for (int i = 0; i < Grd_unlock.Rows.Count; i++)
                    {
                        CheckBox ChAction = (CheckBox)Grd_unlock.Rows[i].FindControl("Chk_UnLockYN");
                        ...
                        for (int j = 0; j < Gridchild.Rows.Count; j++)
                        {
                            ...
                            Chkchild_UnLockYN.Checked = true;
                        }
                    }
                }
                else
                {
                    for (int i = 0; i < Grd_unlock.Rows.Count; i++)
                    {
                        ...
                        for (int j = 0; j < Gridchild.Rows.Count; j++)
                        {
                            ...
                            Chkchild_UnLockYN.Checked = false;
                        }
                    }
                }
