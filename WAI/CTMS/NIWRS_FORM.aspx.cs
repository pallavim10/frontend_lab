...
namespace CTMS
{
    public partial class NIWRS_FORM : System.Web.UI.Page
    {
        ...
        private void GET_FORM()
        {
            try
            {
                ...
                hfSEQNO.Value = ds.Tables[0].Rows[0]["SEQNO"].ToString();
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (CONTROLTYPE == "TEXTBOX")
                    {
                        ...
                        if (READYN == "True")
                        {
                            ...
                            if (VARIABLENAME == "RAND_BRTHDAT" || VARIABLENAME == "RNBRTHDAT")
                            {
                                btnEdit.Text = GET_DOB(hfSUBJID.Value);
                            }
                            else if (VARIABLENAME == "RNSITEID")
                            {
        ...
        private string GET_DOB(string SUBJID)
        {
            string RESULT = "";
            try
            {
                DataSet ds = dal_IWRS.IWRS_DATA_SP(ACTION: "GET_DOB", SUBJID: SUBJID);
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        RESULT = ds.Tables[0].Rows[0]["BRTHDAT"].ToString();
                    }
                }
            ...
            try
            {
                ...
                if (CHECK_OnSubmit_CRITs())
                {
                    if (CHECK_StopClause())
                    {
                        ...
                        Response.Redirect("NIWRS_ACTIONS.aspx?STEPID=" + Request.QueryString["STEPID"].ToString() + "&SUBJID=" + hfSUBJID.Value + "&VISIT=" + hfApplVisit.Value + "");
                    }
                }
            }
            catch (Exception ex)
