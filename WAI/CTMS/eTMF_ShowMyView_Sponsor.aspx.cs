...
namespace CTMS
{
    public partial class eTMF_ShowMyView_Sponsor : System.Web.UI.Page
    {
        DAL dal = new DAL();
            ...
            try
            {
                ...
                else
                {
                    if (!IsPostBack)
                    {
                        ...
                        lblHeader.Text = Request.QueryString["VIEWNAME"].ToString().Replace("'", "");

                        DataSet ds = dal.eTMF_SP(ACTION: "Select_View", ID: Request.QueryString["VIEWID"].ToString());
        ...
        private void Get_Files()
        {
            try
            {
                DataSet ds = dal.eTMF_View_SP(ACTION: "Get_Show_ViewFiles_Sponsor",
                ID: Request.QueryString["VIEWID"].ToString(),
        ...
        private void Get_Files_Count_Tiles()
        {
            try
            {
                DataSet ds = dal.eTMF_View_SP(ACTION: "Get_Show_ViewFiles_Count_Tile",
                ID: Request.QueryString["VIEWID"].ToString(),
                CountryID: drpCountry.SelectedValue,
                SiteID: drpSites.SelectedValue
                );
        ...
        protected void DropDownList1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                dal.eTMF_View_SP(ACTION: "ChnageStatus", ID: ID.Text, Status: drpAction.SelectedValue);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void gvFiles_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (UploadBy == Session["User_ID"].ToString())
                    {
                        lbtnStatus.Visible = true;
                        hdnurl.Value = "eTMF_Change_Status.aspx?DocID=" + ID;
        ...
        protected void gvTreeZone_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_View_SP(ACTION: "GET_VIEW_Sections", UploadZoneId: Folder_ID, ID: Request.QueryString["VIEWID"].ToString());
        ...
        protected void gvTreeSection_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_View_SP(ACTION: "GET_VIEW_Artifacts", UploadSectionId: SubFolder_ID, UploadZoneId: Folder_ID, ID: Request.QueryString["VIEWID"].ToString());
        ...
        protected void gvTreeArtifact_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_View_SP(ACTION: "GET_VIEW_DOCS_Sponsor", RefNo: MainRefNo, ID: Request.QueryString["VIEWID"].ToString());
        ...
        protected void gvTreeDocs_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_View_SP(ACTION: "GET_VIEW_Files", DocID: ID, RefNo: RefNo, UniqueRefNo: UniqueRefNo, CountryID: drpCountry.SelectedValue, SiteID: drpSites.SelectedValue);
