...
namespace CTMS
{
    public partial class NSAE_RESTORE_REQ_LIST : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!this.IsPostBack)
                {
                    DataSet ds = dal_SAE.SAE_DELETED_SP(ACTION: "GET_DELETED_SAE_DETAILS",
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        lblDeletedBy.Text = ds.Tables[0].Rows[0]["DELETEBYNAME"].ToString();
                        lblDeletedDTUTC.Text = ds.Tables[0].Rows[0]["DELETE_CAL_TZDAT"].ToString();
                        lblDeletedDTServer.Text = ds.Tables[0].Rows[0]["DELETE_CAL_DAT"].ToString();
                        lblDeleteReason.Text = ds.Tables[0].Rows[0]["DELETEREASON"].ToString();
                    }
        ...
        protected void btnApproveReq_Click(object sender, EventArgs e)
        {
            try
            {
                dal_SAE.SAE_DELETED_SP(
                    ACTION: "UPDATE_RESTORE_REQ_APPR_ACTION",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text
                    );

                string MESSAGE = "Restore request has been approved Successfully. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), "Approved", "Restore SAE Request (Approve/Disapprove)");

                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('" + MESSAGE.ToString() + "'); window.location='NSAE_RESTORE_REQ.aspx';", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void btnDisApproveReq_Click(object sender, EventArgs e)
        {
            ...
            {
                dal_SAE.SAE_DELETED_SP(
                    ACTION: "UPDATE_RESTORE_REQ_DISSAPPR_ACTION",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text
                    );

                string MESSAGE = "Restore request has been dissapproved. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), "Disapproved", "Restore SAE Request (Approve/Disapprove)");
