...
namespace CTMS
{
    public partial class DM_Mappings : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (Convert.ToString(Request.QueryString["VIEWMAPING"]) == "1")
                    {
                        ...
                        SHOW_CODING_DATA();
                        divSysModule.Visible = false;
                    }
                    else
                    {
                        divSysModule.Visible = true;
                        GetSystems();
                    }
                }
            }
        ...
        protected void GetSystems()
        {
            try
            {
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    hdnSYSTEM.Value = drpSystem.SelectedValue;
                }
                else
                {
        ...
        public void GetModule()
        {
            try
            {
                DataSet ds = dal_DB.DB_ANNOTATED_PRINT_SP(ACTION: "GET_MODULENAME_BY_SYSTEM", SYSTEM: drpSystem.SelectedValue);
        ...
        private void GetStructure(GridView grd)
        {
            try
            {
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    lblModuleName.Text = hdnMODULENAME.Value;
        ...
        protected void btnDownload_Click(object sender, EventArgs e)
        {
            try
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "OpenWindow", "window.open('DB_ANNOTATED_MODULES_PRINT.aspx?MODULEID=" + hdnMODULEID.Value + "&SYSTEM=" + drpSystem.SelectedValue + "','_newtab');", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void btnDownloadAllModules_Click(object sender, EventArgs e)
        {
            try
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "OpenWindow", "window.open('DB_ANNOTATED_MODULES_PRINT.aspx?MODULEID=ALL" + "&SYSTEM=" + drpSystem.SelectedValue + "','_newtab');", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        {
            Response.Redirect("DesignBench_HomePage.aspx?menu=Design%20Bench");
        }
