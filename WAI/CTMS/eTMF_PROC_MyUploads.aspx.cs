...
namespace CTMS
{
    public partial class eTMF_PROC_MyUploads : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void Get_Files_Count_Tiles()
        {
            try
            {
                DataSet ds = dal.eTMF_Process_SP(ACTION: "GET_MY_UPLOADS_TILE_COUNT",
                UploadDocTypeid: drpDocType.SelectedValue,
                UploadZoneId: ddlZone.SelectedValue,
                UploadSectionId: ddlSections.SelectedValue,
                UploadArtifactId: ddlArtifacts.SelectedValue,
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "Collaborate")
                        {
                            lblVal1.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "Review")
                        {
                            lblVal2.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "QC")
                        {
                            lblVal3.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "QA Review")
                        {
                            lblVal4.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "Internal Approval")
                        {
                            lblVal5.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "Sponsor Approval")
                        {
                            lblVal6.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }

                        if (ds.Tables[0].Rows[i]["Status"].ToString() == "Final")
                        {
                            lblVal7.Text = ds.Tables[0].Rows[i]["COUNTS"].ToString();
                        }
                    }
                }
                else
        ...
        protected void gvFiles_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.eTMF_Process_SP(ACTION: "GET_ASSIGNED_USERS", DocID: ID);
