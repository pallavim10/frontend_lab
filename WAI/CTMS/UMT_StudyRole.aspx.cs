    ...
    public partial class UMT_StudyRole : System.Web.UI.Page
    {
        ...
        private void INSERT_STUDYROLE_USER()
        {
            try
            {
                ...
                {
                    Response.Write("<script language=javascript>alert('Please Enter Study Role');</script>");
                    return;
                }
                if (chkInternal.Checked || chkSite.Checked || chkSponsor.Checked || chkExternal.Checked)
                {
                    DataSet da = dal_UMT.UMT_STUDYROLE_SP(
                                            ...
                                            EthicsComm: chkEthicsComm.Checked,
                                            OtherExternal: chkExternal.Checked
                                        );

                    Response.Write("<script> alert('Study role  created successfully'); window.location.href = 'UMT_StudyRole.aspx';</script>");
                ...
                {
                    Response.Write("<script language=javascript>alert('Please select at least one Applicable For');</script>");
                }
            }
            catch (Exception ex)
            {
        ...
        private void UPDATE_STUDYROLE_USER()
        {
            try
            {
                ...
                if (chkInternal.Checked || chkSite.Checked || chkSponsor.Checked || chkExternal.Checked)
                {
                    ...
                    Response.Write("<script> alert('Study Role Updated Successfully'); window.location.href = 'UMT_StudyRole.aspx';</script>");
                }
                else
                {
                    Response.Write("<script language=javascript>alert('Please select at least one Applicable For');</script>");
        ...
        private void DELETE_STUDYROLE_USER(string ID)
        {
            try
            {
                ...
                Response.Write("<script> alert('Study Role deleted Successfully'); window.location.href = 'UMT_StudyRole.aspx';</script>");
                GET_STUDYROLE_USER();
            }
            catch (Exception ex)
            {
        ...
        private void EDIT_STUDYROLE_USER(string ID)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0 && ds.Tables.Count > 0)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["EthicsComm"].ToString() == "True")
                    {
                        chkEthicsComm.Checked = true;
                    }
                    else
                    {
                        chkEthicsComm.Checked = false;
