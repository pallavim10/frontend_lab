...
namespace CTMS
{
    public partial class MM_LISTING_DATA_VISIT : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private DataTable GenerateTransposedTable(DataTable inputTable)
        {
            ...
            for (int i = 0; i < inputTable.Rows.Count; i++)
            {
                ...
                {
                    if (dc.ColumnName != "VISIT" && dc.ColumnName != "VISIT" && dc.ColumnName != "PVID" && dc.ColumnName != "RECID" && dc.ColumnName != "QueryCount" && dc.ColumnName != "ListStatus" && dc.ColumnName != "AnotherListStatus")
                    {
                        if (columns.Contains(inputTable.Rows[i]["VISIT"].ToString()))
                        {
                            DataRow drNew = outputTable.NewRow();
