...
namespace CTMS
{
    public partial class NSAE_LOG_EVENTS_SDV : System.Web.UI.Page
    {
        ...
        public void GetData()
        {
            try
            {
                DataSet ds = dal_SAE.SAE_SDVDETAILS_SP(ACTION: "GET_SAE_LIST_SDV",
        ...
        protected void grdSAELog_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "EditSAE")
                {
                    ...
                    Response.Redirect("NSAE_MODULE_DATA_LOGS_SDV.aspx?INVID=" + INVID + "&SUBJID=" + SUBJID + "&SAE=" + SAE + "&STATUS=" + STATUS + "&SAEID=" + SAEID, false);
                }
            }
            catch (Exception ex)
            {
