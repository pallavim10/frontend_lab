...
namespace CTMS
{
    public partial class Comm_Imported : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void Get_Import()
        {
            try
            {
                DataSet ds = dal.Communication_SP(Action: "Get_Import", UserID: Session["User_ID"].ToString(), PROJECTID: Session["PROJECTID"].ToString(), Reference: txtAutoCompText.Text);

                repeatImported.DataSource = ds;
                repeatImported.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void repeatImported_ItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            try
            {
                ...
                else if (e.CommandName == "Event")
                {
                    Response.Redirect("Comm_NewImportEvent.aspx?ComID=" + ID);
                }
                else if (e.CommandName == "ShowEvent")
                {
                    Response.Redirect("Comm_EventLog.aspx?ComID=" + ID);
                }
        ...
        protected void repeatImported_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    string Event = row["Event"].ToString();
                    ...
                    LinkButton lbtnEvent = (LinkButton)e.Item.FindControl("lbtnEvent");
                    LinkButton lbtnEventCreated = (LinkButton)e.Item.FindControl("lbtnEventCreated");
                    ...
                    if (Event != "0")
                    {
                        lbtnEventCreated.Visible = true;
                    }
                    else
                    {
                        lbtnEventCreated.Visible = false;
                    }
                }
            }
            catch (Exception ex)
