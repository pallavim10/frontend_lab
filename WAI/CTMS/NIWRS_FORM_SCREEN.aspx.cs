...
namespace CTMS
{
    public partial class NIWRS_FORM_SCREEN : System.Web.UI.Page
    {
        ...
        private void GET_SUBECT_DETAILS()
        {
            try
            {
        ...
        private void CHECK_AVL_Screening_ID()
        {
            try
            {
                if (Request.QueryString["SUBJID"] != null)
                {
                    ...
                    ACTION: "CHECK_AVL_Screening_ID_SUBJID",
                    SITEID: drpSite.SelectedValue,
                    SUBSITEID: drpSubSite.SelectedValue,
                    SUBJID: Request.QueryString["SUBJID"].ToString()
                    );

                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        hfAvlScrID.Value = ds.Tables[0].Rows[0]["IDENT"].ToString();
                    }
                    else
                    {
                        if (drpSite.SelectedItem.Text != "--Select--" && drpSubSite.SelectedValue != "")
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Screening ID Not Available'); window.location.href = 'IWRS_Status_Dashboard.aspx'; ", true);
                        }
                    }
                }
                else
                {
                    ...
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        hfAvlScrID.Value = ds.Tables[0].Rows[0]["SUBJID"].ToString();
                    }
                    else
                    {
                        if (drpSite.SelectedItem.Text != "--Select--" && drpSubSite.SelectedValue != "")
        ...
        private void GET_AVL_Screening_ID()
        {
            try
            {
                if (Request.QueryString["SUBJID"] != null)
                {
                    DataSet ds = dal_IWRS.IWRS_AVL_SP(
                    ACTION: "GET_AVL_Screening_ID_SUBJID",
                    SITEID: drpSite.SelectedValue,
                    SUBSITEID: drpSubSite.SelectedValue,
                    SUBJID: Request.QueryString["SUBJID"].ToString()
                    );
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (CONTROLTYPE == "TEXTBOX")
                    {
                        ...
                        if (Request.QueryString["SUBJID"] != null)
                        {
                            if (VARIABLENAME == "MAT_SUBJID")
                            {
                                btnEdit.Text = Request.QueryString["SUBJID"].ToString();
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (CHECK_OnSubmit_CRITs())
                {
                    if (CHECK_StopClause())
                    {
                        GET_AVL_Screening_ID();
                        INSERT_FORM_DATA();
                        UPDATE_SCREENING_DETAILS();

                        Response.Redirect("NIWRS_ACTIONS.aspx?STEPID=" + Request.QueryString["STEPID"].ToString() + "&SUBJID=" + hfAvlScrID.Value + "&VISIT=" + hfApplVisit.Value + "");
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void btncancel_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("IWRS_Status_Dashboard.aspx");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        private bool CHECK_OnSubmit_CRITs()
        {
            ...
            try
            {
                ...
                {
                    DataSet ds = dal_IWRS.IWRS_GET_SUBJECT_DETAILS_SP(ACTION: "GET_SUBJECT_DETAILS", SUBJID: hfAvlScrID.Value);
                    ...
                    if (CASES.Contains("[DATETIME]"))
                    {
                        if (ISDATE(ds.Tables[0].Rows[0]["LATE_DATE"].ToString()))
                        {
                            CASES = CASES.Replace("[LATEVISITDATE]", "CAST('" + cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy hh:mm tt") + "' AS DATETIME)");
                        }
                        else
                        {
                            CASES = CASES.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy hh:mm tt"));
                        }
                    }
        ...
        private void INSERT_FORM_DATA()
        {
            try
            {
                ...
                for (int rownum = 0; rownum < grd_Data.Rows.Count; rownum++)
                {
                    ...
                    if (strdata != "" && Strata != "0")
                    {
                        dal_IWRS.IWRS_DATA_SP(ACTION: "UPDATE_STRATA", VARIABLENAME: varname, ANSWER: strdata, SUBJID: hfAvlScrID.Value);
                    }
                    ...
                    for (int b = 0; b < grd_Data1.Rows.Count; b++)
                    {
                        ...
                        if (CONTROLTYPE != "HEADER" && CONTROLTYPE != "ChildModule")
                        {
                            ...
                            {
                                dal_IWRS.IWRS_DATA_SP(ACTION: "UPDATE_STRATA", VARIABLENAME: varname, ANSWER: strdata1, SUBJID: hfAvlScrID.Value);
                            }
                        }
                        ...
                        for (int c = 0; c < grd_Data2.Rows.Count; c++)
                        {
                            ...
                            if (CONTROLTYPE != "HEADER" && CONTROLTYPE != "ChildModule")
                            {
                                ...
                                {
                                    dal_IWRS.IWRS_DATA_SP(ACTION: "UPDATE_STRATA", VARIABLENAME: varname, ANSWER: strdata2, SUBJID: hfAvlScrID.Value);
                                }
                            }
                            ...
                            for (int d = 0; d < grd_Data3.Rows.Count; d++)
                            {
                                ...
                                if (CONTROLTYPE != "HEADER" && CONTROLTYPE != "ChildModule")
                                {
                                    ...
                                    {
                                        dal_IWRS.IWRS_DATA_SP(ACTION: "UPDATE_STRATA", VARIABLENAME: varname, ANSWER: strdata3, SUBJID: hfAvlScrID.Value);
                                    }
                ...
                {
                    INSERTQUERY = "INSERT INTO [" + hfTablename.Value + "] ([SUBJID_DATA], [SITEID], [ENTEREDBY], [ENTEREDBYNAME], [ENTEREDDAT], [ENTERED_TZVAL], " + COLUMN.Replace("@ni$h", ",") + ") VALUES ('" + hfAvlScrID.Value + "', '" + drpSite.SelectedValue + "', '" + Session["User_ID"].ToString() + "','" + Session["User_Name"].ToString() + "', GETDATE(), '" + Session["TimeZone_Value"].ToString() + "', " + DATA.Replace("@ni$h", ",") + " )";
                }
                else
                {
                    INSERTQUERY = "INSERT INTO [" + hfTablename.Value + "] ([SUBJID_DATA], [SITEID], [ENTEREDBY], [ENTEREDBYNAME], [ENTEREDDAT], [ENTERED_TZVAL], " + COLUMN.Replace("@ni$h", ",") + ") VALUES ('" + hfAvlScrID.Value + "', '" + drpSite.SelectedValue + "', '" + Session["User_ID"].ToString() + "','" + Session["User_Name"].ToString() + "', GETDATE(), '" + Session["TimeZone_Value"].ToString() + "', " + DATA.Replace("@ni$h", ",") + " )";
                ...
                UPDATEQUERY = UPDATEQUERY + " WHERE SUBJID_DATA = '" + hfAvlScrID.Value + "' ";
                ...
                foreach (DataRow drModules in dsMODULEs.Tables[0].Rows)
                {
                    ...
                    if (dsVisit.Tables[0].Rows.Count > 0)
                    {
                        ...
                        string PVID = Session["PROJECTID"].ToString() + "-" + drpSite.SelectedValue + "-" + hfAvlScrID.Value + "-" + VISITID + "-" + SYNC_MODULEID + "-1";
                        string RECID = "0";
                        ...
                        syncUPDATEQUERY = syncUPDATEQUERY + " WHERE PVID = '" + PVID + "' AND RECID = '" + RECID + "' AND SUBJID_DATA = '" + hfAvlScrID.Value + "' ";
                ...
                dal_IWRS.IWRS_SYNC_SP(ACTION: "UPDATE_DM_AUDITTRAIL_FIELDNAME", SUBJID: hfAvlScrID.Value);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        private void UPDATE_SCREENING_DETAILS()
        {
            try
            {
                dal_IWRS.IWRS_ACTIONS_SP(ACTION: "UPDATE_SCREENING_DETAILS", SITEID: drpSite.SelectedValue, SUBJID: hfAvlScrID.Value, CurrentDate: Session["IWRS_CurrentDate"].ToString());
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
