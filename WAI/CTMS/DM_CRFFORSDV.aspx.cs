...
namespace CTMS
{
    public partial class DM_CRFFORSDV : System.Web.UI.Page
    {
        ...
        protected void Grd_OpenCRF_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "GOTOPAGE")
                {
                    ...
                    if (drpSubID.SelectedValue == "Select")
                    {
                        throw new Exception("Select Subject Id");
                    }

                    if ((gvr.FindControl("MULTIPLEYN") as Label).Text == "True")
                    {
                        Response.Redirect("DM_DataSDV.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID + "&MULTIPLEYN=1");
                    }
                    else
                    {
                        Response.Redirect("DM_DataSDV.aspx?MODULEID=" + MODULEID + "&MODULENAME=" + MODULENAME + "&VISITID=" + VISITID + "&VISIT=" + VISIT + "&INVID=" + INVID + "&SUBJID=" + SUBJID + "&MULTIPLEYN=0");
                    }
        ...
        protected void lbtnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Write("<script> alert('Rules successfully refreshed.');window.location='DM_CRFFORSDV.aspx?VISITNUM=" + drpVisit.SelectedValue + "&SUBJID=" + drpSubID.SelectedValue + "'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
