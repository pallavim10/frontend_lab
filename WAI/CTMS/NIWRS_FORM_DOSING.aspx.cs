...
namespace CTMS
{
    public partial class NIWRS_FORM_DOSING : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "calcAge();", true);
                }
            }
            catch (Exception ex)
            {
        ...
        private void GET_AutoPopList()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_DATA_SP(ACTION: "GET_AutoPopList", STEPID: Request.QueryString["STEPID"].ToString(), SUBJID: hfSUBJID.Value);
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    repeatAutoPopList.DataSource = outputTable;
                    repeatAutoPopList.DataBind();
                }
            }
            catch (Exception ex)
            {
        ...
        private void GET_SUBECT_DETAILS()
        {
            try
            {
                ...
                hfSITEID.Value = ds.Tables[0].Rows[0]["SITEID"].ToString();
                hfSUBSITEID.Value = ds.Tables[0].Rows[0]["SUBSITEID"].ToString();
                hfSUBJID.Value = ds.Tables[0].Rows[0]["SUBJID"].ToString();
                hfINDICATION.Value = ds.Tables[0].Rows[0]["INDICATION"].ToString();
                hfINDICATION_ID.Value = ds.Tables[0].Rows[0]["INDICATION_ID"].ToString();
                hfRANDID.Value = ds.Tables[0].Rows[0]["RAND_NUM"].ToString();
                hfVISIT.Value = ds.Tables[0].Rows[0]["NEXT_VISIT"].ToString();
                hfVISITNUM.Value = ds.Tables[0].Rows[0]["VISITNUM"].ToString();
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        private void GET_AVL_KITS()
        {
            try
            {
                DataSet dsCheckArm = dal_IWRS.IWRS_KITS_SP(ACTION: "CHECK_ARMS_Criteria", SUBJID: hfSUBJID.Value, STEPID: Request.QueryString["STEPID"].ToString());
                ...
                if (dsCheckArm.Tables.Count > 0 && dsCheckArm.Tables[0].Rows.Count > 0 && dsCheckArm.Tables[0].Rows[0]["TREAT_GRP"].ToString() != "0")
                {
                    foreach (DataRow dr in dsCheckArm.Tables[0].Rows)
                    {
                        ...
                        DataSet ds = dal_IWRS.IWRS_AVL_SP(ACTION: "GET_AVL_KITS_CRITERIA", TREAT_GRP: dr["TREAT_GRP"].ToString(), TREAT_STRENGTH: "", SITEID: hfSITEID.Value, QUANTITY: Quantity.ToString());

                        if (ds.Tables[0].Rows.Count < 1 || ds.Tables[0].Rows.Count < Quantity)
                        {
                            ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "alert('Insufficient Kits in Inventory'); window.location.href = 'IWRS_Status_Dashboard.aspx'; ", true);
                        }
                        else
                        {
                            if (IsPostBack)
                            {
                                Response.Redirect("NIWRS_ACTIONS.aspx?STEPID=" + Request.QueryString["STEPID"].ToString() + "&SUBJID=" + hfSUBJID.Value + "&VISIT=" + hfApplVisit.Value + "&DOSEVISIT=" + hfVISIT.Value + "&DOSEVISITNUM=" + hfVISITNUM.Value + "&DOSEINDIC=" + hfINDICATION_ID.Value + "");
                            }
                        }
                    }
                }
                ...
                {
                    DataSet dsArm = dal_IWRS.IWRS_KITS_SP(ACTION: "GET_ARMS", SUBJID: hfSUBJID.Value, STEPID: Request.QueryString["STEPID"].ToString());

                    foreach (DataRow dr in dsArm.Tables[0].Rows)
                    {
                        DataSet ds = dal_IWRS.IWRS_AVL_SP(ACTION: "GET_AVL_KITS", TREAT_GRP: dr["TREAT_GRP"].ToString(), TREAT_STRENGTH: dr["TREAT_STRENGTH"].ToString(), SITEID: hfSITEID.Value);
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (CONTROLTYPE == "TEXTBOX")
                    {
                        ...
                        if (READYN == "True")
                        {
                            ...
                            if (VARIABLENAME == "EXREGIME")
                            {
                                btnEdit.Text = GET_CRF_DATA(VARIABLENAME);
                            }
                        }
                        if (MULTILINEYN == "True")
                        {
        ...
        private DataTable GET_GRID_DATATABLE()
        {
            ...
            DataSet dsAddFields = dal_IWRS.IWRS_DATA_SP(ACTION: "GET_AddFields_DATA", SUBJID: Request.QueryString["SUBJID"].ToString());
            ...
            DataSet dsDETAILS = dal_IWRS.IWRS_GET_SUBJECT_DETAILS_SP(ACTION: "GET_SUBJECT_DETAILS", SUBJID: Request.QueryString["SUBJID"].ToString(), FORMID: hfMODULEID.Value);
        ...
        private string GET_CRF_DATA(string VARIABLENAME)
        {
            string RESULT = "";
            try
            {
                DataSet ds = dal_IWRS.IWRS_DATA_SP(ACTION: "GET_CRF_DATA", VARIABLENAME: VARIABLENAME, SUBJID: hfSUBJID.Value);
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        RESULT = ds.Tables[0].Rows[0]["DATA"].ToString();
                    }
                }
            }
            catch (Exception ex)
