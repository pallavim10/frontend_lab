...
namespace CTMS
{
    public partial class NSAE_CLOSED_SAE_DETAILS : System.Web.UI.Page
    {
            ...
            try
            {
                if (!this.IsPostBack)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            btnReOpenSAE.Visible = false;
                        }
                        else
                        {
                            btnReOpenSAE.Visible = true;
                        }
                    }
        ...
        protected void btnSubmitReOpenSAE_Click(object sender, EventArgs e)
        {
            try
            {
                dal_SAE.SAE_CLOSE_SP(
                    ACTION: "UPDATE_REOPEN_SAE_REQ",
                    SAEID: lblSAEID.Text,
                    REASON: txtReason.Text
                    );

                string MESSAGE = "Re-Open request has been generated Successfully. ";

                SendEmail(Request.QueryString["SAEID"].ToString(), lblStatus.Text, "Generate Re-Open Request");
