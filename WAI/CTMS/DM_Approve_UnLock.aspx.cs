...
namespace CTMS
{
    public partial class DM_Approve_UnLock : System.Web.UI.Page
    {
        ...
        protected void drpInvID_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                Grd_AppUnlock.DataSource = null;
                Grd_AppUnlock.DataBind();
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.ToString();
        ...
        protected void Btn_Get_Data_Click(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "GET_UNLOCK_REQUEST_DATA",
                ...
                if (ds.Tables[0] != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        Grd_AppUnlock.DataSource = ds.Tables[0];
                        Grd_AppUnlock.DataBind();
                    }
                    else
                    {
        ...
        protected void Grd_AppUnlock_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (MULTIPLEYN.Text == "True")
                    {
                        ...
                        DataSet ds = dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "GET_MULTIPLE_RECORDS_UNLOCK_REQUEST",
                            PVID: lblPVID.Text,
        ...
        protected void Btn_Update_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < Grd_AppUnlock.Rows.Count; i++)
                {
                    CheckBox Chk_ReqApp = (CheckBox)Grd_AppUnlock.Rows[i].FindControl("Chk_ReqApp");

                    Label lblSUBJID = (Label)Grd_AppUnlock.Rows[i].FindControl("lblSUBJID");

                    Label lblVISITNUM = (Label)Grd_AppUnlock.Rows[i].FindControl("lblVISITNUM");

                    Label lblVISIT = (Label)Grd_AppUnlock.Rows[i].FindControl("lblVISIT");

                    Label lblMODULEID = (Label)Grd_AppUnlock.Rows[i].FindControl("lblMODULEID");

                    Label lblMODULENAME = (Label)Grd_AppUnlock.Rows[i].FindControl("lblMODULENAME");

                    Label lblPVID = (Label)Grd_AppUnlock.Rows[i].FindControl("lblPVID");

                    Label MULTIPLEYN = (Label)Grd_AppUnlock.Rows[i].FindControl("MULTIPLEYN");

                    Label lblTableName = (Label)Grd_AppUnlock.Rows[i].FindControl("lblTableName");

                    Label lblRECID = (Label)Grd_AppUnlock.Rows[i].FindControl("lblRECID");

                    Label PAGESTATUS = (Label)Grd_AppUnlock.Rows[i].FindControl("PAGESTATUS");

                    if (PAGESTATUS.Text == "2")
                    {
                        if (Chk_ReqApp.Checked)
                        {
                            dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "UPDATE_UNLOCK_REQUEST_APPROVED",
                                PVID: lblPVID.Text,
                                RECID: lblRECID.Text,
                                INVID: drpInvID.SelectedValue,
                                SUBJID: lblSUBJID.Text,
                    ...
                    else
                    {
                        if (MULTIPLEYN.Text == "True")
                        {
                            GridView Gridchild = (GridView)Grd_AppUnlock.Rows[i].FindControl("Gridchild");
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('UnLock Request Approved Successfully'); window.location.href='DM_Approve_UnLock.aspx'", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
        ...
        protected void Btn_DisApprove_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < Grd_AppUnlock.Rows.Count; i++)
                {
                    CheckBox Chk_ReqDisApp = (CheckBox)Grd_AppUnlock.Rows[i].FindControl("Chk_ReqDisApp");
                    ...
                    if (PAGESTATUS.Text == "2")
                    {
                        ...
                        {
                            dal_DM.DM_UNLOCK_REQUEST_SP(ACTION: "UPDATE_UNLOCK_REQUEST_DISAPPROVED",
                                PVID: lblPVID.Text,
                                RECID: lblRECID.Text,
                                INVID: drpInvID.SelectedValue,
                                SUBJID: lblSUBJID.Text,
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('UnLock Request Disapproved Successfully'); window.location.href='DM_Approve_UnLock.aspx'", true);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = "";
