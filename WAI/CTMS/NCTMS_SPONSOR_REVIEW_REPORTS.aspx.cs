...
namespace CTMS
{
    public partial class NCTMS_SPONSOR_REVIEW_REPORTS : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void grdField_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.GetSetChecklistComments(Action: "GET_CHECKLIST_COMMENTS_SPONSOR",
            ...
            try
            {
                ...
                if (drpModule.SelectedIndex != 0)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            Response.Redirect("NCTMS_SPONSOR_REVIEW_REPORTS.aspx?MODULEID=" + drpModule.SelectedValue + "&MODULENAME=" + drpModule.SelectedItem.Text + "&VISITID=" + Request.QueryString["VISITID"] + "&VISIT=" + Request.QueryString["VISIT"] + "&INVID=" + Request.QueryString["INVID"] + "&VISIT_NOM=" + Request.QueryString["VISIT_NOM"].ToString() + "&SVID=" + Request.QueryString["SVID"].ToString());
                        }
                    ...
                    {
                        Response.Redirect("NCTMS_SPONSOR_REVIEW_REPORTS.aspx?MODULEID=" + drpModule.SelectedValue + "&MODULENAME=" + drpModule.SelectedItem.Text + "&VISITID=" + Request.QueryString["VISITID"] + "&VISIT=" + Request.QueryString["VISIT"] + "&INVID=" + Request.QueryString["INVID"] + "&VISIT_NOM=" + Request.QueryString["VISIT_NOM"].ToString());
                    }
                }
                else
                {
        ...
        protected void btnBacktoHomePage_Click(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect("NCTMS_SPONSOR_REVIEW_OPEN_CRF.aspx?SVID=" + hdnSVID.Value + "&SITEID=" + hdnSITEID.Value + "&VISITID=" + hdnVISITID.Value + "&VISIT=" + hdnVISIT.Value, false);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
