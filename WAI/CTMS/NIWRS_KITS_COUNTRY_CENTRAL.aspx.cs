...
namespace CTMS
{
    public partial class NIWRS_KITS_COUNTRY_CENTRAL : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_TREATMENT()
        {
            try
            {
                ...
                {
                    DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_TREATMENTS_COUNTRY_CENTRAL",
        ...
        private void GET_KITS_COUNTRY_ORDER()
        {
            try
            {
                ...
                for (int i = 0; i < repeatTreat.Items.Count; i++)
                {
                    ...
                    if (txtNoKits.Text != "")
                    {
                        DataSet dsKits = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_KITS_COUNTRY_CENTRAL", TREAT_GRP: lblTreatment.Text, KITNO: txtNoKits.Text, COUNTRYID: ddlCountry.SelectedValue);
                ...
                if (kits)
                {
                    ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_KITS_COUNTRY_CENTRAL", TREAT_GRP: "0", KITNO: "", COUNTRYID: ddlCountry.SelectedValue);
                }
        ...
        protected void btnGetKits_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "ALERT", "alert(' Please Select Country ID.'); ", true);
                }
            }
            catch (Exception ex)
            {
        ...
        private void GET_NEW_ORDERID_COUNTRY()
        {
            try
            {
                if (ddlCountry.SelectedValue != "0" )
                {
                    DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_NEW_CENTRAL_ORDERID_COUNTRY", COUNTRYID: ddlCountry.SelectedValue);
        ...
        private void GENERATE_ORDER_COUNTRY()
        {
            try
            {
                bool Checked = false;
                ...
                for (int i = 0; i < gvKITS.Rows.Count; i++)
                {
                    ...
                    if (Chk_Sel.Checked)
                    {
                        ...
                        dal_IWRS.IWRS_KITS_COUNTRY_SP
                           (
                           ACTION: "GENERATE_CENTRAL_ORDER_COUNTRY",
                           ID: ID.Value,
                           COUNTRYID: ddlCountry.SelectedValue,
        ...
        private void SEND_MAIL()
        {
            try
            {
                ...
                DataSet ds = dal_IWRS.NIWRS_SETUP_SP(ACTION: "GET_EMAILS_BYCOUNTRY", STRATA: "KIT_COUNTRY", ID: ddlCountry.SelectedValue);
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_COUNTRY_CENTRAL");
