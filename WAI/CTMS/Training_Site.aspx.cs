...
namespace CTMS
{
    public partial class Training_Site : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void get_Trainee()
        {
            try
            {
                DataSet ds = dal.Train_Verification_SP(Action: "get_Trainees_Site", Project_ID: Session["PROJECTID"].ToString());
                ddlTrainee.DataSource = ds.Tables[0];
                ddlTrainee.DataValueField = "ID";
        ...
        private void get_Sections()
        {
            try
            {
                ...
                {
                    DataSet ds = dal.Train_Verification_SP(Action: "get_Plan_Section_Site", Project_ID: Session["PROJECTID"].ToString(), Plan_ID: ddlPlan.SelectedValue);
        ...
        protected void gvSections_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.Train_Verification_SP(Action: "get_Plan_SubSection_Site", Plan_ID: ddlPlan.SelectedValue, Project_ID: Session["PROJECTID"].ToString(), Section_ID: Section_ID, Emp_ID: ddlTrainee.SelectedValue);
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (SubSectionIDs != "")
                {
                    Response.Redirect("Train_Verification_Site.aspx?SubSec=" + SubSectionIDs + "&Plan_ID=" + ddlPlan.SelectedValue + "&Emp_ID=" + ddlTrainee.SelectedValue + "");
                }
            }
            catch (Exception ex)
            {
