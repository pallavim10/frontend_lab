...
namespace CTMS
{
    public partial class eTMF_VERSION_HISTORY : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                else
                {
                    if (!this.IsPostBack)
                    {
                        ...
                        DataSet ds = dal_eTMF.eTMF_DATA_SP(ACTION: "GET_VERSION_HISTORY", ID: Request.QueryString["ID"].ToString());
                        MainDs.Merge(ds);
                        ...
                        do
                        {
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                {
                                    ds = new DataSet();
                                    ds = dal_eTMF.eTMF_DATA_SP(ACTION: "GET_VERSION_HISTORY", ID: ID);
                                    MainDs.Merge(ds);
                                }
                            }
                        }
                        while (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count == 1);

                        gvFiles.DataSource = MainDs;
                        gvFiles.DataBind();
                    }
                }
            }
        ...
        protected void gvFiles_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    string DELETED = drv["DELETE"].ToString();
