...
namespace CTMS
{
    public partial class Comm_Trails : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void Get_Trail()
        {
            try
            {
                DataSet ds = dal.Communication_SP(Action: "Get_Trail", UserID: Session["User_ID"].ToString(), PROJECTID: Session["PROJECTID"].ToString(), Reference: txtAutoCompText.Text, UNIQUEID: Request.QueryString["ComId"].ToString());

                repeatTrails.DataSource = ds;
                repeatTrails.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void repeatTrails_ItemCommand(object sender, RepeaterCommandEventArgs e)
        {
            try
            {
        ...
        protected void repeatTrails_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
