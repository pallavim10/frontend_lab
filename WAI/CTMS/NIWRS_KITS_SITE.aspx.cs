...
namespace CTMS
{
    public partial class NIWRS_KITS_SITE : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_KITS()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_KITS_SITE", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        private void SEND_MAIL_QUARANTINE(string KITNO, string COUNTRY, string SITE)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE-Quaratine");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[QUARANTINED_AT]", "Site");
        ...
        private void SEND_MAIL_BLOCK(string KITNO, string COUNTRY, string SITE)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE - Block");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[SITE]", SITE);
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[Blocked_AT]", "Site");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_DAMAGE(string KITNO, string COUNTRY, string SITE)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE - Damage");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[SITE]", SITE);
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[Damaged_AT]", "Site");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
        ...
        private void SEND_MAIL_RETENTION(string KITNO, string COUNTRY, string SITE)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE -Retention");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    BODY = BODY.Replace("[SITE]", SITE);
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[Retention_AT]", "Site");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
                ...
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    if (Chek_Quarantine.Checked)
                    {
                        dal_IWRS.IWRS_KITS_SITE_SP(
                            ACTION: "QUARANTINE_SITE_INVENTORY",
                            KITNO: KITNO,
        ...
        private void ExportSingleSheet()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_SITE_SP(ACTION: "GET_KITS_SITE_EXPORT", COUNTRYID: ddlCountry.SelectedValue, SITEID: ddlSite.SelectedValue);
        ...
        protected void btnSubmitRetention_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    {
                        dal_IWRS.IWRS_KITS_SITE_SP(
                            ACTION: "RETENTION_SITE_INVENTORY",
                            KITNO: KITNO,
                            RETENTIONCOMM: txtRetentionComments.Text
                            );

                        SEND_MAIL_RETENTION(KITNO, COUNTRYNAME, SITEID);
                    }
                }
        ...
        protected void btnSubmitExpire_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    {
                        dal_IWRS.IWRS_KITS_SITE_SP(
                            ACTION: "EXPIRED_SITE_INVENTORY",
                            KITNO: KITNO,
                            EXPIRECOMM: txtExpireComments.Text
                            );

                        SEND_MAIL_EXPIRE(KITNO, COUNTRYNAME, SITEID);
                    }
                }
        ...
        private void SEND_MAIL_EXPIRE(string KITNO, string COUNTRY,string SITEID)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_KITS_SITE_INVENTORY -Expire");
                ...
                if (dsEmail.Tables.Count > 0 && dsEmail.Tables[0].Rows.Count > 0)
                {
                    ...
                    SUBJECT = SUBJECT.Replace("[PROJECTID]", Session["PROJECTIDTEXT"].ToString());
                    SUBJECT = SUBJECT.Replace("[COUNTRY]", COUNTRY);
                    SUBJECT = SUBJECT.Replace("[SITE]", SITEID);
                    SUBJECT = SUBJECT.Replace("[KITNO]", KITNO);
                    SUBJECT = SUBJECT.Replace("[USERNAME]", Session["User_Name"].ToString());
                    SUBJECT = SUBJECT.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    SUBJECT = SUBJECT.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                    ...
                    BODY = BODY.Replace("[PROJECTID]", Session["PROJECTIDTEXT"].ToString());
                    BODY = BODY.Replace("[COUNTRY]", COUNTRY);
                    BODY = BODY.Replace("[SITE]", SITEID);
                    BODY = BODY.Replace("[KITNO]", KITNO);
                    BODY = BODY.Replace("[Expired_AT]", "Site Depot");
                    BODY = BODY.Replace("[USERNAME]", Session["User_Name"].ToString());
                    BODY = BODY.Replace("[DATETIME]", cf.GetCurrentDateTimeByTimezone().ToString("dd-MMM-yyyy HH:mm"));
                    BODY = BODY.Replace("[TIMEZONE]", Session["TimeZone_Value"].ToString());
                }
