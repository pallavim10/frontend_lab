...
namespace CTMS
{
    public partial class NIWRS_UNBLIND_REJ : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    grdUNBLINDREJ.Columns[2].HeaderText = Session["SUBJECTTEXT"].ToString();
                    GetSites();
                    GET_SUBSITE();
                    GET_DATA();
                }
        ...
        private void GET_DATA()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_UNBLIND_SP(ACTION: "GET_UNBLIND_REJECT_LIST", SITEID: drpSite.SelectedValue, SUBSITEID: drpSubSite.SelectedValue);
                grdUNBLINDREJ.DataSource = ds;
                grdUNBLINDREJ.DataBind();
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
