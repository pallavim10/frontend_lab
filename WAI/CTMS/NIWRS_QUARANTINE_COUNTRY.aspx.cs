...
namespace CTMS
{
    public partial class NIWRS_QUARANTINE_COUNTRY : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void GET_KITS()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_QUARANTINE_COUNTRY", COUNTRYID: ddlCountry.SelectedValue);
        ...
        private void SEND_MAIL_UNQUARANTINE(string ORDERID, string COUNTRY, string KITNO)
        {
            try
            {
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_COUNTRY - UnQuarantine");
        ...
        private void SEND_MAIL_REJECT(string ORDERID, string COUNTRY, string KITNO,string Location)
        {
            try
            {
                CommonFunction.CommonFunction cf = new CommonFunction.CommonFunction();
                ...
                string EMAILIDS = "", CCEMAILIDS = "", BCCEMAILIDS = "", SUBJECT = "", BODY = "";
                DataSet ds = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAILS", STRATA: "KIT_COUNTRY_REJECT",COUNTRY: COUNTRY);
                ...
                DataSet dsEmail = dal_IWRS.IWRS_SET_EMAIL_SP(ACTION: "GET_EMAIL_TEMPLATE", STRATA: "NIWRS_QUARANTINE_COUNTRY - Reject");
        ...
        protected void btnSubmitReject_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    {
                        dal_IWRS.IWRS_KITS_COUNTRY_SP(
                                ACTION: "REJECT_COUNTRY_KIT",
                                KITNO: KITNO,
                                REJECTCOMM: txtRejectComments.Text,
                                TABLENAME: TABLENAME
                            );

                        SEND_MAIL_REJECT(ORDERID, COUNTRYNAME, KITNO, Location);
                    }
                }
        ...
        private void ExportSingleSheet()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_KITS_COUNTRY_SP(ACTION: "GET_QUARANTINE_COUNTRY_EXPORT", COUNTRYID: ddlCountry.SelectedValue);
        ...
        protected void btnSubmitExpire_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < gvKits.Rows.Count; i++)
                {
                    ...
                    {
                        dal_IWRS.IWRS_KITS_COUNTRY_SP(
                            ACTION: "EXPIRE_QURANTITNE_COUNTRY",
                            KITNO: KITNO,
