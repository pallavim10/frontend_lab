...
namespace CTMS
{
    public partial class ETMF_Upload_Docs : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void gvMainTask_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.Budget_SP(Action: "get_DocUpload_Tasks", Project_Id: Session["PROJECTID"].ToString(), Task_ID: Task_ID);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
