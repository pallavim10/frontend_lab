<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="Budget_ProjectPlan.aspx.cs" Inherits="CTMS.Budget_ProjectPlan" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function ShowAudit(element) {
            ...
            var Task_ID = $(element).closest('tr').find('td:eq(2)').find('span').html(); //unique id for each row
            var Task = $(element).closest('tr').find('td:eq(4)').find('span').html(); //unique id for each row
            var Sub_Task_ID = $(element).closest('tr').find('td:eq(3)').find('span').html(); //unique id for each row
