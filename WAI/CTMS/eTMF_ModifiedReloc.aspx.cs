...
namespace CTMS
{
    public partial class eTMF_ModifiedReloc : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        private void Get_Files()
        {
            try
            {
                DataSet ds = dal_eTMF.eTMF_REPORT_SP(
                    ...
                    UPLOADBY: ddlUplodedBy.SelectedValue,
                    CountryID: drpCountry.SelectedValue,
                    SiteID: drpInvID.SelectedValue,
                    ModifReloc: drpModifReloc.SelectedValue,
        ...
        protected void gvFiles_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                DataSet ds = dal_eTMF.eTMF_DATA_SP(ACTION: "Download_File", ID: e.CommandArgument.ToString(), CountryID: drpCountry.SelectedValue,
                        SiteID: drpInvID.SelectedValue);
        ...
        protected void UPLOADEDBY_USERS()
        {
            try
            {
                DataSet ds = dal_eTMF.eTMF_REPORT_SP(ACTION: "GET_UPLODED_USERS");

                if (ds.Tables[0].Rows.Count > 0)
                {
                    ddlUplodedBy.DataSource = ds;
                    ddlUplodedBy.DataTextField = "UPLOADBYNAME";
                    ddlUplodedBy.DataValueField = "UPLOADBY";
                    ddlUplodedBy.DataBind();
                    ddlUplodedBy.Items.Insert(0, new ListItem("--Select--", "0"));
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                DataSet ds = dal_eTMF.eTMF_REPORT_SP(
                    ACTION: "GET_ModifReloc_DOCS_Export",
                    ZONEID: ddlZone.SelectedValue,
                    SECTIONID: ddlSections.SelectedValue,
                    ARTIFACTID: ddlArtifacts.SelectedValue,
                    DocID: drpDocument.SelectedValue,
                ...
                //DataTable CopieTable = ds.Tables[0].Copy();
                //if (CopieTable.Columns.Contains("FILENAME_Link"))
                //{
                //    CopieTable.Columns.Remove("FILENAME_Link");
                //}
                //dsExport.Tables.Add(CopieTable);

                ds.Tables[0].TableName = lblHeader.Text;

                Multiple_Export_Excel.ToExcel_WithHyperlink(ds, lblHeader.Text + ".xls", Page.Response, LinkValuePairs);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void btnExportPDF_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Multiple_Export_Excel.ToPDF_WithHyperlink(ds, lblHeader.Text, Page.Response);
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
