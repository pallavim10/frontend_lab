...
namespace CTMS
{
    public partial class CTMS_CRA_ProjectTimeline : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void gvTasks_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.Budget_SP(Action: "Audit_TrailExist_CRA", Project_Id: Session["PROJECTID"].ToString(), Task_ID: Task_Id, Sub_Task_ID: Sub_Task_Id, INVID: ddl_INVID.SelectedValue);
        ...
        protected void ddl_INVID_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddl_INVID.SelectedIndex != 0)
                {
                    ...
                    gvMainTask.Visible = true;
                }
                else
                {
                    btnsubmit.Visible = false;
                    gvMainTask.Visible = false;
                }
            }
            catch (Exception ex)
            {
        ...
        private void bind_Dept()
        {
            try
            {
                DataSet ds = dal.Budget_SP(Action: "get_CRA_Dept_TOM", Project_Id: Session["PROJECTID"].ToString());
                if (ds.Tables[0].Rows.Count > 0)
                {
        ...
        protected void gvMainTask_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    DataSet ds = dal.Budget_SP(Action: "get_CRA_project_Plans", Project_Id: Session["PROJECTID"].ToString(), INVID: ddl_INVID.SelectedValue, Task_ID: Task_ID);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
        ...
        private void GETDATA()
        {
            try
            {
                DataSet ds = dal.Budget_SP(Action: "get_CRA_Task", Dept_Id: ddlDepartment.SelectedValue);
