...
namespace CTMS
{
    public partial class NIWRS_UNBLIND_REQ_LIST : System.Web.UI.Page
    {
        DAL dal = new DAL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    grdUNBLINDREQs.Columns[2].HeaderText = Session["SUBJECTTEXT"].ToString();
                    GetSites();
                    GET_SUBSITE();
                    GET_DATA();
                }
        ...
        private void GET_DATA()
        {
            try
            {
                DataSet ds = dal_IWRS.IWRS_UNBLIND_SP(ACTION: "GET_UNBLIND_REQ_LIST", SITEID: drpSite.SelectedValue, SUBSITEID: drpSubSite.SelectedValue);
                grdUNBLINDREQs.DataSource = ds;
                grdUNBLINDREQs.DataBind();
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
        ...
        protected void grdUNBLINDREQs_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            string ID = e.CommandArgument.ToString();

            if (e.CommandName == "Open Req")
            {
                Response.Redirect("NIWRS_UNBLIND_REQ_APPREJ.aspx?ID=" + ID);
            }
        }
    }
}